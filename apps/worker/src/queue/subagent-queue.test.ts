...
function staleCandidateEligible(candidate: StaleCandidate): boolean {
  ...
  return (candidate.status === "queued" || candidate.status === "retrying")
    ...
    && candidate.key === subagentDispatchKey(candidate.id)
}
...
function staleRepairPool(options: StaleRepairOptions = {}) {
  ...
  const dispatches: StaleDispatch[] = candidates.map(candidate => ({
    id: `dispatch-${candidate.id}`, taskId: candidate.id, sessionId: candidate.sessionId, key: candidate.key ?? subagentDispatchKey(candidate.id),
    ...
    publishedAt: candidate.publishedAt ?? null, attemptCount: 4, lastError: "old-error" as string | null,
  }))
  const calls: Array<[string, unknown[]?]> = []
  let snapshot: typeof dispatches | null = null
  const client = {
    query: vi.fn(async (sql: string, params?: unknown[]) => {
      calls.push([sql, params])
      if (sql === "BEGIN") {
...
describe("stale subagent dispatch recovery", () => {
  ...
  it("uses updatedAt/id order and clamps work to the requested limit", async () => {
    const early = staleCandidate({ id: "task-early", updatedAt: new Date("2026-09-13T00:00:00.000Z"), payload: { ...payload, taskId: "task-early", ownerId: "old-owner" }, key: subagentDispatchKey("task-early") })
    const late = staleCandidate({ id: "task-late", updatedAt: new Date("2026-09-14T00:00:00.000Z"), payload: { ...payload, taskId: "task-late", ownerId: "old-owner" }, key: subagentDispatchKey("task-late") })
