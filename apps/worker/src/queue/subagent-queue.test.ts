...
function recoveredDispatchPool() {
  ...
  const client = {
    query: vi.fn(async (sql: string, params?: unknown[]) => {
      ...
      if (sql.startsWith('INSERT INTO "agent_outbox"')) {
        outbox.payload = JSON.parse(String(params?.[4])) as unknown
  ...
  return { pool: { connect: vi.fn().mockResolvedValue(client) } as unknown as pg.Pool, calls, outbox, get staleResetCount() { return staleResetCount } }
}
...
describe("Subagent queue", () => {
  ...
  it.each(["running", "paused", "waiting_for_user"] as const)("resets an open %s session dispatch", async sessionStatus => {
    ...
    expect(fake.calls[lockIndex]?.[0]).not.toContain('controlGate')
    expect(fake.calls[insertIndex]?.[0]).toContain("WHERE EXISTS")
    expect(fake.calls[insertIndex]?.[0]).not.toContain('controlGate')
  ...
  it("dispatches pending intents and marks them published only after queue add", async () => {
    ...
    expect(scan?.[0]).not.toContain('controlGate')
  ...
  it("repairs a missing queued dispatch before the normal dispatcher publishes it", async () => {
    ...
    expect(fake.calls[sessionLockIndex]?.[0]).not.toContain('controlGate')
    const insert = fake.calls.find(([sql]) => sql.startsWith('INSERT INTO "agent_outbox"'))
    expect(insert?.[0]).toContain('ON CONFLICT ("idempotencyKey") DO NOTHING')
    expect(insert?.[0]).not.toContain('controlGate')
    expect(insert?.[1]?.[2]).toBe("session-1")
  ...
  it("repairs before dispatch when invoked through recoverSubagentQueue", async () => {
    ...
    const staleIndex = fake.calls.findIndex(([sql]) => sql.includes('FROM "sub_agent_tasks" AS task') && sql.includes("FOR UPDATE OF task, dispatch"))
    const repairIndex = fake.calls.findIndex(([sql]) => sql.includes('FROM "sub_agent_tasks" AS task') && sql.includes("FOR UPDATE OF task SKIP LOCKED"))
    ...
    expect(staleIndex).toBeLessThan(repairIndex)
    expect(repairIndex).toBeGreaterThan(-1)
    expect(repairIndex).toBeLessThan(dispatchIndex)
    expect(queue.add).toHaveBeenCalledTimes(1)
  })

  it("does not stale-reset a dispatch already reset by manager recovery", async () => {
    const fake = recoveredDispatchPool()
    const queue = { add: vi.fn().mockResolvedValue(undefined) }
    const manager = {
      recover: vi.fn().mockResolvedValue({
        rows: [{ id: "task-1", sessionId: "session-1", rootTaskId: "root-1", status: "queued" }], reclaimed: 1, terminal: 0,
      }),
    } as unknown as AgentTreeManager
    await expect(recoverSubagentQueue(fake.pool, queue, manager, 1)).resolves.toMatchObject({ reclaimed: 1, terminal: 0, repaired: 0, dispatched: 1 })
    expect(fake.staleResetCount).toBe(0)
    expect(queue.add).toHaveBeenCalledTimes(1)
    expect(fake.outbox.publishedAt).not.toBeNull()
    expect((fake.outbox.payload as SubagentJobPayload).ownerId).toMatch(/^recovery-/)
    const staleScan = fake.calls.findIndex(([sql]) => sql.includes('dispatch."publishedAt" IS NOT NULL'))
    const missingScan = fake.calls.findIndex(([sql]) => sql.includes('dispatch."id" IS NULL'))
    const pendingScan = fake.calls.findIndex(([sql]) => sql.includes('ORDER BY dispatch."createdAt"'))
    expect(staleScan).toBeGreaterThan(-1)
    expect(staleScan).toBeLessThan(missingScan)
    expect(missingScan).toBeLessThan(pendingScan)
  })
