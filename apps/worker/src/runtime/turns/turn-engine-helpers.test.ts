...
describe("TurnEngine helpers", () => {
  ...
  it("subtracts durable tool calls from the resumed tool budget", () => {
    const resume = { nextOrdinal: 1, stepCount: 1, toolCallCount: 2, inputThroughSequence: 1n, consumedInputIds: [], usage: { inputTokens: 0, outputTokens: 0, estimatedCostUsd: 0 } }
    expect(resumedBudgetLimits({ maxToolCalls: 5 }, resume)).toEqual({ maxToolCalls: 3 })
    expect(resumedBudgetLimits({ maxToolCalls: 2 }, resume)).toEqual({ maxToolCalls: 0 })
  })
})
