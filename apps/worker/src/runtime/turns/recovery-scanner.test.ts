...
function pool(rows: unknown[] = [], sessionStatus: string | null = "running", queuedRows: unknown[] = [], legacyRows: unknown[] = [], lineageRows: readonly unknown[] = [{ id: "turn_1" }]) {
...
describe("Turn recovery scanner", () => {
  ...
  it("reclaims only stale in-progress rows and increments their fence", async () => {
    ...
    expect(sql).not.toContain('controlGate')
  ...
  it.each([
    ...
    const fake = pool([{ id: "outbox_poison", aggregateId: "session_1", payload }], "running", [], [], lineageRows)
  ...
  it("repairs queued or reclaimed work even when the queue add is unavailable", async () => {
    ...
    expect(guardedDispatchInserts.every(([sql]) => sql.includes('turn."sessionId" = $3') && sql.includes('session."status" NOT IN'))).toBe(true)
  })
