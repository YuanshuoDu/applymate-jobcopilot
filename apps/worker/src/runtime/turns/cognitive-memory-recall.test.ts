...
function memory(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    revisions: { goalRevision: 3, planRevision: null },
    decisions: [], jobEvidenceExcerpts: [], unresolvedQuestions: [], unresolved: [], waits: [], approvals: [], verifiedEvidence: [], artifacts: [], taskRefs: [], eventRefs: [], omittedRanges: [{ fromId: "event:a", toId: "event:b", reason: "compaction" }], coveredSequence: "5", ...overrides,
  }
}
...
describe("cognitive memory recall", () => {
  ...
  it("recalls server-classified public job excerpts as quoted untrusted data", () => {
    const value = memory({ jobEvidenceExcerpts: [{
      ...
      excerpt: "company=Example | role=Engineer | location=Dublin | url=https://jobs.example/1 | source=greenhouse | salary=€70k", goalRevision: 3,
    }] })
  ...
  it("drops foreign, private-field, and URL-bearing forged excerpts", () => {
    const foreign = memory({ jobEvidenceExcerpts: [{ id: "job-2", referenceId: "job-2", sourceRef: "tool-result:jobs-1", toolName: "jobs.search", trust: "external_untrusted", fields: { company: "Foreign", role: "Engineer" }, excerpt: "company=Foreign | role=Engineer", goalRevision: 3 }] })
    ...
    const malformed = memory({ jobEvidenceExcerpts: [{ id: "job-1", referenceId: "job-1", sourceRef: "tool-result:jobs-1", toolName: "jobs.get", trust: "external_untrusted", fields: { company: "Example", role: "Engineer", url: "https://jobs.example/1?token=secret" }, excerpt: "company=Example | role=Engineer | url=https://jobs.example/1?token=secret", goalRevision: 3 }] })
  ...
  it("drops an excerpt when a retained tool observation disagrees with its binding", () => {
    const value = memory({ jobEvidenceExcerpts: [{
      id: "job-1", referenceId: "job-1", sourceRef: "tool-result:jobs-1", toolName: "jobs.get", trust: "external_untrusted",
      fields: { company: "Example", role: "Engineer" }, excerpt: "company=Example | role=Engineer", goalRevision: 3,
    }] })
  ...
  it("recalls only sanitized contact markers from persisted job evidence", () => {
    const value = memory({ jobEvidenceExcerpts: [{
      ...
      excerpt: "company=Example [REDACTED_EMAIL] | role=Engineer | description=Call [REDACTED_PHONE]", goalRevision: 3,
    }] })
    ...
    const unsanitized = memory({ jobEvidenceExcerpts: [{
      id: "job-1", referenceId: "job-1", sourceRef: "tool-result:jobs-1", toolName: "jobs.get", trust: "external_untrusted",
      fields: { company: "Example recruiter@example.com", role: "Engineer" }, excerpt: "company=Example recruiter@example.com | role=Engineer", goalRevision: 3,
    }] })
  ...
  it("omits memory with a mismatched goal revision or malformed schema", () => {
    const mismatch = memory({ revisions: { goalRevision: 2, planRevision: null } })
  ...
  it("bounds and sorts reference metadata without leaking unsupported statuses", () => {
    const eventRefs: CognitiveMemoryRecallReference[] = Array.from({ length: 24 }, (_, index) => ({ id: `event:${String(index).padStart(2, "0")}`, status: index === 0 ? "pending" : "model-secret", goalRevision: 3, sequence: String(index) }))
    ...
    expect(recall.references.eventRefs.metadata[0]).toMatchObject({ id: "event:00", status: "pending", goalRevision: 3, sequence: "0" })
