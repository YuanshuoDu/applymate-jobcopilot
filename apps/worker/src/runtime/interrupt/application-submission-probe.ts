...
import type { ApplicationSubmissionStartScope } from "../../db/application-task-state.js"
...
export async function isApplicationSubmissionStopped(pool: Pool, scope: ApplicationSubmissionStartScope): Promise<boolean> {
  ...
  try {
    ...
    const session = await client.query<{ status: string }>(
      `SELECT "status" FROM "agent_sessions" WHERE "id" = $1 AND "userId" = $2`,
    ...
    if (!sessionStatus || !turnStatus) throw new Error("Application submission Stop scope is unavailable.")
    return isStopped(sessionStatus, turnStatus, interrupted.rows[0]?.stopped)
  } catch (error: unknown) {
...
function isStopped(sessionStatus: string, turnStatus: string, interruptedEvent: boolean | undefined): boolean {
  return ["aborted", "archived"].includes(sessionStatus) || turnStatus === "interrupted" || interruptedEvent === true
}
