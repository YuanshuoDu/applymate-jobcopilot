...
describe("context memory schema", () => {
  it("keeps generic waits scoped without deriving planning narrative", () => {
    const value = projection()
    expect(value.decisions).toEqual([])
    expect(value.unresolvedQuestions).toEqual([])
    expect(value.waits).toEqual([{ id: "wait-result:wait-1", status: "waiting", goalRevision: 2, sequence: "6" }])
    expect(value.unresolved).toEqual([{ id: "wait-result:wait-1", status: "waiting", goalRevision: 2, sequence: "6" }])
  ...
  it("drops old planning narrative while preserving current goal-scoped waits", () => {
    ...
    expect(current?.unresolvedQuestions).toEqual([])
    expect(current?.waits.map(item => item.id)).toEqual(["wait-result:wait-1"])
    expect(current?.unresolved.map(item => item.id)).toEqual(["wait-result:wait-1"])
  })
