...
export function referenceScope(value: CognitiveMemoryReference, expectedGoalRevision: number | undefined): "current" | "stale" | "future" {
  if (expectedGoalRevision === undefined || value.goalRevision === undefined || value.goalRevision === expectedGoalRevision) return "current"
  if (value.goalRevision !== undefined && value.goalRevision > expectedGoalRevision) return "future"
  return "stale"
}

function excerptScope(value: CognitiveMemoryJobEvidenceExcerpt, expectedGoalRevision: number | undefined): "current" | "stale" | "future" {
  return referenceScope(value, expectedGoalRevision)
}
...
export function mergePriorMemory(content: Row, maps: readonly Map<string, CognitiveMemoryReference>[], jobEvidence: Map<string, CognitiveMemoryJobEvidenceExcerpt>, ranges: CognitiveMemoryOmittedRange[], expectedGoalRevision: number | undefined): bigint | null | false {
  const normalized = validateContextMemoryProjection(content.memory, expectedGoalRevision === undefined ? {} : { expectedGoalRevision })
