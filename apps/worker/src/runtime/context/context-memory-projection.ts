...
import { validateContextMemoryProjection, type CognitiveMemoryAnchor, type CognitiveMemoryJobEvidenceExcerpt, type CognitiveMemoryOmittedRange, type CognitiveMemoryReference, type ContextMemoryProjection } from "./context-memory-schema.js"
...
function reference(observation: Observation, content: Row): CognitiveMemoryReference | null {
  const result: { id: string; status?: string; goalRevision?: number; sequence?: string } = { id: observation.id }
...
function isCriticalId(id: string): boolean {
  return ["wait-result:", "approval:", "artifact:", "task:", "event:"].some(prefix => id.startsWith(prefix))
}
...
export function isContextMemoryAnchorObservation(observation: Observation): boolean {
  ...
  return content?.kind === "context_snapshot_memory" || isWaitToolName(content?.toolName) || content?.approvalId !== undefined
}
...
export function buildContextMemoryProjection(snapshot: StepContextSnapshot, options: ContextMemoryProjectionOptions = {}): ContextMemoryProjection | null {
  ...
  const jobEvidence = new Map<string, CognitiveMemoryJobEvidenceExcerpt>()
  ...
  for (const item of observations) {
    ...
    if (scope === "current") {
      ...
      if (["waiting", "pending", "running", "retrying", ...FAILURE_STATUSES].includes(current.status ?? "")) if (!add(unresolved, current)) return null
    }
    ...
    if (kind === "context_summary" && content.memory !== undefined) {
      const prior = mergePriorMemory(content, [unresolved, waits, approvals, verified, artifacts, tasks, events], jobEvidence, omittedRanges, expectedGoalRevision)
  ...
  const cleanRefs = (map: Map<string, CognitiveMemoryReference>): CognitiveMemoryReference[] => [...map.values()].map(({ planRevision: _planRevision, goalRevision: _goalRevision, ...item }) => ({ ...item, ...(expectedGoalRevision === undefined || _goalRevision === undefined ? {} : { goalRevision: _goalRevision }) }))
  const cleanEvidence = [...jobEvidence.values()].map(({ planRevision: _planRevision, goalRevision: evidenceGoalRevision, ...item }) => ({ ...item, ...(expectedGoalRevision === undefined || evidenceGoalRevision === undefined ? {} : { goalRevision: evidenceGoalRevision }) }))
  const goalRevision = goalContent?.revision as number | undefined ?? null
  const projection: ContextMemoryProjection = { schemaVersion: "agent-harness.cognitive-memory.v1", activeGoals: activeGoals.slice(0, MAX_ITEMS), fixedConstraints: fixedConstraints.slice(0, MAX_ITEMS), steering: userSteering.slice(0, MAX_ITEMS), revisions: { goalRevision, planRevision: null }, decisions: [], unresolvedQuestions: [], jobEvidenceExcerpts: sorted(cleanEvidence).slice(0, 8), unresolved: sorted(cleanRefs(unresolved)).slice(0, MAX_ITEMS), waits: sorted(cleanRefs(waits)).slice(0, MAX_ITEMS), approvals: sorted(cleanRefs(approvals)).slice(0, MAX_ITEMS), verifiedEvidence: sorted(cleanRefs(verified)).slice(0, MAX_ITEMS), artifacts: sorted(cleanRefs(artifacts)).slice(0, MAX_ITEMS), taskRefs: sorted(cleanRefs(tasks)).slice(0, MAX_ITEMS), eventRefs: sorted(cleanRefs(events)).slice(0, MAX_ITEMS), omittedRanges: omittedRanges.sort((left, right) => left.fromId.localeCompare(right.fromId)).slice(0, MAX_ITEMS), coveredSequence: covered?.toString() ?? null }
  const trimmed = trimProjection(projection, maxBytes)
  return trimmed ? validateContextMemoryProjection(trimmed, { maxBytes }) : null
}
