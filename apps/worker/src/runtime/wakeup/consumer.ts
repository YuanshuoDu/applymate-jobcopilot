...
class TerminalWakeupError extends Error {
  constructor(readonly code: TerminalWakeupErrorCode, message: string) {
    super(message)
    this.name = "TerminalWakeupError"
  }
}
...
async function assertWakeupEvent(client: Client, payload: AgentTurnWakeupPayload): Promise<void> {
  const result = await client.query<WakeupEventRow>(
    `SELECT "sessionId", "turnId", "itemId", "type", "payload" FROM "agent_events" WHERE "id" = $1 AND "sessionId" = $2 AND "turnId" = $3 AND "itemId" = $4 AND "type" = 'turn.wakeup' FOR SHARE`,
    [payload.eventId, payload.sessionId, payload.turnId, payload.itemId],
  )
...
async function appendResumeEvent(client: Client, payload: AgentTurnWakeupPayload, userId: string): Promise<void> {
  const sequenceResult = await client.query<{ eventSequence: string | bigint }>(
    `UPDATE "agent_sessions" SET "eventSequence" = "eventSequence" + 1 WHERE "id" = $1 AND "userId" = $2 RETURNING "eventSequence"`,
...
async function resumeInTransaction(client: Client, payload: AgentTurnWakeupPayload): Promise<WakeupResult> {
  ...
  const turnResult = await client.query<{ userId: string; status: string; revision: number }>(
    `SELECT turn."userId", turn."status", turn."revision" FROM "agent_turns" AS turn INNER JOIN "agent_sessions" AS session ON session."id" = turn."sessionId" AND session."userId" = turn."userId" WHERE turn."id" = $1 AND turn."sessionId" = $2 AND session."id" = $2 FOR UPDATE`,
  ...
  const updated = await client.query(
    `UPDATE "agent_turns" SET "status" = 'queued', "revision" = "revision" + 1, "completedAt" = NULL, "updatedAt" = CURRENT_TIMESTAMP WHERE "id" = $1 AND "sessionId" = $2 AND "userId" = $4 AND "status" IN ('waiting_for_approval', 'waiting_for_user') AND "revision" = $3`,
    [payload.turnId, payload.sessionId, payload.nextTurnRevision, turn.userId],
  )
  ...
  await appendResumeEvent(client, payload, turn.userId)
...
async function selectPendingWakeups(pool: PoolLike, batchSize: number): Promise<OutboxRow[]> {
  return withTransaction(pool, async (client) => (await client.query<OutboxRow>(`SELECT "id", "aggregateId", "payload" FROM "agent_outbox" WHERE "topic" = $1 AND "publishedAt" IS NULL ORDER BY "createdAt" ASC, "id" ASC FOR UPDATE SKIP LOCKED LIMIT $2`, [AGENT_TURN_WAKEUP_TOPIC, batchSize])).rows)
}
