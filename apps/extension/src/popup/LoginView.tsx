...
type LoginLabels = {
  ...
  loginError: string; sidePanelError: string; genericError: string; oauthTimeout: string; yourAiJobCopilot?: string; waitingForBrowser?: string; cancel?: string; emailPlaceholder?: string
}

const LABELS: Record<string, LoginLabels> = {
  en: { connectTitle: 'Connect to ApplyMate AI', connectSub: 'Sign in to sync all your jobs automatically', emailLabel: 'Email', pwLabel: 'Password', loginBtn: 'Sign in', loggingIn: 'Signing in…', noAccount: "Don't have an account?", signupLink: 'Sign up free →', googleLogin: 'Sign in with Google', githubLogin: 'Sign in with GitHub', orEmail: 'or sign in with email', openSidebar: 'Open sidebar', loginError: 'Sign in failed', genericError: 'Something went wrong.', oauthTimeout: 'OAuth sign-in timed out. Your previous session was kept.', sidePanelError: 'Chrome could not open the side panel. Use the Side panel button in the toolbar.', yourAiJobCopilot: 'Your AI job copilot', waitingForBrowser: 'Waiting for login in browser tab…', cancel: 'Cancel', emailPlaceholder: 'you@example.com' },
  zh: { connectTitle: '连接 ApplyMate AI', connectSub: '登录后自动同步所有职位', emailLabel: '邮箱', pwLabel: '密码', loginBtn: '登录', loggingIn: '登录中…', noAccount: '还没有账号？', signupLink: '免费注册 →', googleLogin: '使用 Google 登录', githubLogin: '使用 GitHub 登录', orEmail: '或使用邮箱登录', openSidebar: '打开侧边栏', loginError: '登录失败', genericError: '出了点问题，请重试。', oauthTimeout: 'OAuth 登录超时，已保留之前的会话。', sidePanelError: 'Chrome 无法打开侧边栏，请使用浏览器工具栏中的侧边栏按钮。', yourAiJobCopilot: '你的 AI 求职助手', waitingForBrowser: '等待浏览器标签页登录……', cancel: '取消', emailPlaceholder: '你的邮箱@example.com' },
}

export function getPopupLang(): PopupLang {
  try {
    return localStorage.getItem('applymate_lang') === 'zh' ? 'zh' : 'en'
...
export function LoginView({ settings, labels, onLogin }: { settings: ExtensionSettings; labels: LoginLabels; onLogin: (settings: ExtensionSettings) => void }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => { if (!stopped) { stopped = true; setOauthPending(false); setError(labels.oauthTimeout) } }, 30_000)
  ...
  async function handleLogin(event: React.FormEvent) {
    ...
    } catch (caught) { setError(getPopupLang() === 'zh' ? labels.genericError : caught instanceof Error ? caught.message : labels.loginError) }
  ...
  return <div style={{ background: C.bg, minHeight: 360, color: C.navy }}>
    <header style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderBottom: `1px solid ${C.border}` }}>
      ...
      <div><div style={{ fontSize: 15, fontWeight: 750 }}>ApplyMate AI</div><div style={{ marginTop: 2, fontSize: 10, color: C.muted }}>{labels.yourAiJobCopilot ?? 'Your AI job copilot'}</div></div>
    </header>
    <main style={{ padding: '20px 20px 22px' }}>
      ...
      {oauthPending ? <div style={{ textAlign: 'center', padding: '16px 12px', background: C.lavender, borderRadius: 10, border: `1px solid ${C.border}`, color: C.muted, fontSize: 12 }}><div className="am-spin" style={{ width: 16, height: 16, margin: '0 auto 8px', border: '2px solid rgba(79,70,229,0.2)', borderTopColor: C.primary, borderRadius: '50%' }} />{labels.waitingForBrowser ?? 'Waiting for login in browser tab…'}<br /><button type="button" onClick={() => setOauthPending(false)} style={linkStyle}>{labels.cancel ?? 'Cancel'}</button></div> : <>
        ...
        <form onSubmit={handleLogin} style={{ display: 'grid', gap: 10 }}><Input label={labels.emailLabel} type="email" value={email} onChange={setEmail} placeholder={labels.emailPlaceholder ?? 'you@example.com'} /><Input label={labels.pwLabel} type="password" value={password} onChange={setPassword} placeholder="••••••••" />{error && <div role="alert" style={{ fontSize: 11, color: '#B54747', padding: '7px 10px', background: '#FFF2F2', borderRadius: 7 }}>{error}</div>}<Button type="submit" disabled={loading} primary>{loading ? labels.loggingIn : labels.loginBtn}</Button></form>
