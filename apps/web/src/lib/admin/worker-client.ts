...
export async function sendWorkerCommand(command: Omit<WorkerCommand, 'timestamp' | 'nonce'>) {
  ...
  const result = await response.json().catch(() => null) as { receipt?: string; queues?: unknown; error?: string } | null
