...
export function toPlanCatalogDto(input: unknown): PlanCatalogDto {
  ...
  const plan = planKey(row.plan ?? row.key)
  const monthlyPriceCents = integerValue(row.monthlyPriceCents ?? row.priceMinor)
  const yearlyPriceCents = integerValue(row.yearlyPriceCents ?? monthlyPriceCents * 12)
  const entitlements = Array.isArray(row.entitlements) ? row.entitlements.flatMap((item, index) => {
    try { return [{ ...toEntitlementDto(item), id: `entitlement_${index + 1}` }] }
    catch { return [] }
  }) : []
  return {
    id: stringValue(row.id) || `plan_${plan}`, plan, name: stringValue(row.name), description: optionalText(row.description, 500),
    monthlyPriceCents, yearlyPriceCents,
    currency: stringValue(row.currency).toUpperCase(), active: row.active === true, version: integerValue(row.version) || 1, entitlements,
  }
}
...
export function toEntitlementDto(input: unknown): EntitlementValue & { id: string } {
  if (typeof input === 'string') {
    ...
    const featureKey = separator < 0 ? input : input.slice(0, separator)
    const suffix = separator < 0 ? '' : input.slice(separator + 1)
    const value = !suffix
      ? validateEntitlement({ featureKey, kind: 'boolean', enabled: true })
      : /^\d+(?:\/.*)?$/.test(suffix)
        ? validateEntitlement({ featureKey, kind: 'limit', enabled: true, limit: Number(suffix.split('/')[0]) })
        : validateEntitlement({ featureKey, kind: 'text', enabled: true, textValue: suffix })
    return { id: `entitlement_${featureKey}`, ...value }
  }
  const row = record(input)
