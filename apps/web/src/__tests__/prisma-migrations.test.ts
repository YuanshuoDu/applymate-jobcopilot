...
describe('Prisma migration dependencies', () => {
  ...
  it('appends audit hashes from the chain leaf instead of createdAt order', () => {
    const migrationPath = join(migrationsRoot, '20260907110000_repair_admin_audit_chain_append_order', 'migration.sql')
    const migration = readFileSync(migrationPath, 'utf8')

    expect(migration).toContain('INTO STRICT previous')
