...
const mocks = vi.hoisted(() => ({ requireAdmin: vi.fn(), snapshot: vi.fn(), audit: vi.fn() }))
...
describe('GET /api/admin/v1/observability', () => {
  beforeEach(() => { vi.resetModules(); mocks.requireAdmin.mockReset(); mocks.snapshot.mockReset(); mocks.audit.mockReset() })
  ...
  it('returns no-store aggregates only for an authorized actor', async () => {
    ...
    await expect(response.json()).resolves.toEqual({ overall: { total: 3 }, byAts: [] })
