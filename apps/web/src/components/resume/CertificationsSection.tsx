...
export function CertificationsSection({ certifications, onChange, dragHandleProps, onRemove }: {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <SectionHeader
        title={t('resume.section.certifications')}
      ...
      {!collapsed && (
        <>
          {certifications.length === 0 && (
            <div style={{ fontSize: 12, color: 'var(--text-muted)', padding: '8px 4px' }}>{t('resume.noCertifications')}</div>
          )}

          {certifications.map((c, i) => (
            editIdx === i ? (
              <div key={i} style={{ border: '0.5px solid var(--primary)', borderRadius: 6, padding: 10, marginBottom: 8 }}>
                <InlineInput value={c.name}   onChange={v => { const n=[...certifications]; n[i]={...n[i],name:v};   onChange(n) }} placeholder={t('resume.certificationName')} style={{ marginBottom: 6 }} />
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 6 }}>
                  <InlineInput value={c.issuer} onChange={v => { const n=[...certifications]; n[i]={...n[i],issuer:v}; onChange(n) }} placeholder={t('resume.issuingOrganization')} />
                  <InlineInput value={c.date}   onChange={v => { const n=[...certifications]; n[i]={...n[i],date:v};   onChange(n) }} placeholder={t('resume.datePlaceholder')} />
                </div>
                <InlineInput value={c.url??''} onChange={v => { const n=[...certifications]; n[i]={...n[i],url:v||undefined}; onChange(n) }} placeholder={t('resume.credentialUrl')} />
              ...
              <div key={i}
                ...
                <div style={{ flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }} onClick={() => setEditIdx(i)}>
                  <div>
                    <span style={{ fontSize: 12, fontWeight: 500 }}>{c.name || <em style={{ color: 'var(--text-muted)' }}>{t('resume.untitled')}</em>}</span>
                    {c.issuer && <span style={{ fontSize: 12, color: 'var(--text-muted)' }}> · {c.issuer}</span>}
                    {c.url && <a href={c.url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ fontSize: 10, color: 'var(--primary)', marginLeft: 8 }}>↗ {t('resume.verify')}</a>}
                  </div>
