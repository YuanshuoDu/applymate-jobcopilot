      ...
      {!collapsed && (
        <div className={flash ? 'ai-flash-highlight' : ''} style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {skills.map((s, i) => {
            ...
            return (
              <span key={`${s}-${i}`} style={{ fontSize: 11, background: isMatch ? 'rgba(5,150,105,0.10)' : 'var(--bg-secondary)', color: isMatch ? 'var(--c-success)' : 'var(--text)', border: `0.5px solid ${isMatch ? 'rgba(5,150,105,0.20)' : 'var(--border)'}`, borderRadius: 5, padding: '3px 8px', display: 'flex', alignItems: 'center', gap: 4 }}>
                {isMatch && <span style={{ fontSize: 8, color: 'var(--c-success)' }}>✓</span>}
                {s}
