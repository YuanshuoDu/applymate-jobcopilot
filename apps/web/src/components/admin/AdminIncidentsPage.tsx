...
export function AdminIncidentsPage({ canManage }: { canManage: boolean }) {
  ...
  const { t } = useI18n()
  const severityLabel = (value: string) => t(`admin.incidents.severityLabel.${value}`)
  const statusLabel = (value: string) => t(`admin.incidents.status.${value}`)
