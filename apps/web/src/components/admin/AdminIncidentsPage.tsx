...
export function AdminIncidentsPage({ canManage }: { canManage: boolean }) {
  ...
  useEffect(() => { void load() }, [])
  async function create(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); const reason = await request({ title: 'Create incident', label: 'Incident reason', kind: 'reason' }); if (!reason) return; const response = await fetch('/api/admin/v1/incidents', { method: 'POST', headers: adminMutationHeaders(), body: JSON.stringify({ title, summary, service, severity, reason }) }); const payload = await response.json().catch(() => null) as { error?: string } | null; setNotice(response.ok ? 'Incident created.' : payload?.error ?? 'Unable to create incident.'); if (response.ok) { setTitle(''); setSummary(''); await load() } }
  async function update(id: string, status: string) { const reason = await request({ title: `Set incident to ${status}`, label: 'Change reason', kind: 'reason' }); if (!reason) return; const response = await fetch(`/api/admin/v1/incidents/${id}`, { method: 'PATCH', headers: adminMutationHeaders(), body: JSON.stringify({ status, reason }) }); const payload = await response.json().catch(() => null) as { error?: string } | null; setNotice(response.ok ? 'Incident updated.' : payload?.error ?? 'Unable to update incident.'); if (response.ok) await load() }
