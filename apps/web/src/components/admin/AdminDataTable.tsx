...
export function AdminDataTable({ title, subtitle, endpoint, columns, searchable = false, searchLabel, searchPlaceholder, filters = [], exportEndpoint, refreshMs = 30_000, bulkActions = [] }: { title: string; subtitle: string; endpoint: string; columns: Column[]; searchable?: boolean; searchLabel?: string; searchPlaceholder?: string; filters?: Filter[]; exportEndpoint?: string; refreshMs?: number; bulkActions?: BulkAction[] }) {
  ...
  const { data, loading, error } = useApi<{ items: TableItem[]; nextCursor: string | null }>(url, { timeoutMs: 10_000 })
