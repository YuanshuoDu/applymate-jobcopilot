...
export function AdminSubscriptionControls({ canUpdate }: { canUpdate: boolean }) {
  ...
  return <Card style={{ padding: 20, display: 'grid', gap: 16 }}><div><h2 style={{ margin: 0, fontSize: 16 }}>{t('subscription.title')}</h2><p style={{ margin: '6px 0 0', color: 'var(--text-muted)', fontSize: 12 }}>{t('subscription.description')}</p></div>{notice && <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>{notice}</div>}<form onSubmit={(event) => void save(event)} style={{ display: 'grid', gridTemplateColumns: 'minmax(180px, 1.4fr) repeat(3, minmax(120px, 1fr)) auto', gap: 10, alignItems: 'end' }}><label style={labelStyle}>{t('subscription.userId')}<input required value={userId} onChange={(event) => setUserId(event.target.value)} style={inputStyle} /></label><label style={labelStyle}>{t('admin.plan')}<select value={plan} onChange={(event) => setPlan(event.target.value)} style={inputStyle}><option value="free">Free</option><option value="pro">Pro</option><option value="enterprise">Enterprise</option></select></label><label style={labelStyle}>{t('subscription.state')}<select value={status} onChange={(event) => setStatus(event.target.value)} style={inputStyle}><option value="trialing">{t('subscription.trialing')}</option><option value="active">{t('admin.active')}</option><option value="past_due">{t('subscription.pastDue')}</option><option value="cancelled">{t('subscription.cancelled')}</option><option value="expired">{t('subscription.expired')}</option></select></label><label style={labelStyle}>{t('subscription.trialEnds')}<input type="datetime-local" required={status === 'trialing'} value={trialEndsAt} onChange={(event) => setTrialEndsAt(event.target.value)} style={inputStyle} /></label><Btn variant="primary" disabled={!canUpdate}>{t('common.save')}</Btn></form><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>{t('admin.user')}</th><th>{t('admin.plan')}</th><th>{t('subscription.state')}</th><th>{t('subscription.trialEnd')}</th><th>{t('subscription.periodEnd')}</th></tr></thead><tbody>{items.length === 0 ? <tr><td colSpan={5}>{t('subscription.empty')}</td></tr> : items.map(item => <tr key={item.id}><td>{item.user.name ?? t('admin.unnamed')} · {item.user.email}</td><td>{item.plan}</td><td>{item.status}{item.cancelAtPeriodEnd ? ` · ${t('subscription.ending')}` : ''}</td><td>{item.trialEndsAt ? new Date(item.trialEndsAt).toLocaleString() : '—'}</td><td>{item.currentPeriodEnd ? new Date(item.currentPeriodEnd).toLocaleString() : '—'}</td></tr>)}</tbody></table></div></Card>
}
