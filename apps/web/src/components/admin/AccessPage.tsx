...
export function AccessPage() {
  ...
  if (loading) return <div style={{ color: 'var(--text-muted)' }}>Loading access controls…</div>
  return <div style={{ maxWidth: 1180, margin: '0 auto' }}>
    <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 22 }}><div><div style={{ color: '#5b6b80', fontSize: 11, textTransform: 'uppercase', letterSpacing: '.08em' }}>Security</div><h1 style={{ margin: '5px 0 0', fontSize: 26 }}>Access management</h1><p style={{ margin: '6px 0 0', color: '#5b6b80' }}>Edit roles and review administrator sessions.</p></div><button type="button" onClick={() => void load()} title="Refresh access data" style={{ border: '1px solid #ccd7e3', background: '#fff', color: '#31475f', padding: 9, borderRadius: 6, cursor: 'pointer' }}><RefreshCw size={16} aria-hidden="true" /></button></header>
    ...
    <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}><button type="button" onClick={() => setTab('roles')} aria-pressed={tab === 'roles'} style={tabButton(tab === 'roles')}>Role editor</button><button type="button" onClick={() => setTab('matrix')} aria-pressed={tab === 'matrix'} style={tabButton(tab === 'matrix')}>Permission matrix</button></div>
    {tab === 'roles' ? <>
      <section style={sectionStyle}><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}><label style={labelStyle}>Role<select value={selectedId} onChange={event => setSelectedId(event.target.value)} style={inputStyle}>{roles.map(role => <option key={role.id} value={role.id}>{role.name} {role.system ? '(system)' : ''}</option>)}</select></label><label style={labelStyle}>New key<input value={newKey} onChange={event => setNewKey(event.target.value)} placeholder="custom_support" style={inputStyle} /></label><label style={labelStyle}>Name<input value={name} onChange={event => setName(event.target.value)} style={inputStyle} /></label></div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16, alignItems: 'end', flexWrap: 'wrap' }}><label style={{ ...labelStyle, flex: 1, minWidth: 220 }}>Reason<input value={reason} onChange={event => setReason(event.target.value)} placeholder="Why is this access change needed?" style={inputStyle} /></label>{newKey.trim() ? <button type="button" onClick={() => void createRole()} disabled={saving} style={primaryButton}><Plus size={14} aria-hidden="true" /> Create role</button> : <button type="button" onClick={() => void saveRole()} disabled={saving || !selected} style={primaryButton}><Check size={14} aria-hidden="true" /> Save role</button>}</div>
      </section>
