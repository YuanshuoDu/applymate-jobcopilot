...
export function AdminQueuesPage({ permissions }: { permissions: readonly string[] }) {
  ...
  async function retry(queue: string, job: FailedJob) {
    if (!job.id) return
    const reason = window.prompt(`Retry ${job.id}: enter operational reason`)
    if (!reason) return
