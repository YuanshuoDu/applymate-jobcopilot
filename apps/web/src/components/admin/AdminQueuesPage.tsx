...
export function AdminQueuesPage({ permissions }: { permissions: readonly string[] }) {
  ...
  return <><div className="admin-page"><header className="admin-header"><div><h1>Queues</h1><p>Safe queue controls without task payload access</p></div><ServerCog size={22} aria-hidden="true" /></header><section className="admin-list-page"><div className="admin-queue-title"><span>{notice}</span><button className="admin-row-action" title="Refresh queue summary" onClick={() => void load()}><RefreshCw size={16} /></button></div>{worker && <section className="admin-status-panel"><strong>Worker {worker.state === 'paused' ? 'paused' : worker.status === 'ok' ? 'healthy' : 'unavailable'}</strong><span>{worker.workerId} · release {worker.version} · uptime {Math.floor(worker.uptimeSeconds / 3600)}h {Math.floor(worker.uptimeSeconds / 60) % 60}m</span><div className="admin-inline-actions"><button className="admin-row-action" title="Resume all worker queues" disabled={workerBusy || !canResume || worker.state !== 'paused'} onClick={() => void changeWorker('resume')}><Play size={15} /> Resume Worker</button><button className="admin-row-action" title="Pause all worker queues" disabled={workerBusy || !canPause || worker.state === 'paused'} onClick={() => void changeWorker('pause')}><Pause size={15} /> Pause Worker</button></div></section>}<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Queue</th><th>Waiting</th><th>Active</th><th>Delayed</th><th>Failed</th><th>Stuck</th><th>State</th><th aria-label="Actions" /></tr></thead><tbody>{queues.filter(queue => queue.name !== 'dead-letter').map((queue) => <tr key={queue.name}><td>{queue.name}</td><td>{queue.counts.waiting ?? 0}</td><td>{queue.counts.active ?? 0}</td><td>{queue.counts.delayed ?? 0}</td><td>{queue.counts.failed ?? 0}</td><td>{queue.stuckActiveCount ?? 0}</td><td>{queue.paused ? 'Paused' : 'Running'}</td><td><button className="admin-row-action" title="View failed jobs" disabled={!permissions.includes('queues.read')} onClick={() => void loadFailed(queue.name)}>Failed</button>{queue.paused ? <button className="admin-row-action" title="Resume queue" disabled={!canResume || worker?.state === 'paused'} onClick={() => void change(queue, 'resume')}><Play size={15} /></button> : <button className="admin-row-action" title="Pause queue" disabled={!canPause} onClick={() => void change(queue, 'pause')}><Pause size={15} /></button>}</td></tr>)}</tbody></table></div>{deadLetter && <section className="admin-status-panel"><strong>Dead-letter queue · {deadLetter.counts.waiting ?? 0} waiting</strong><span>Tasks that exhausted their configured retry attempts remain here until an operator retries them.</span><div className="admin-inline-actions"><button className="admin-row-action" disabled={!permissions.includes('queues.read')} onClick={() => void loadFailed('dead-letter')}>View dead-letter jobs</button></div></section>}{failedQueue && <section className="admin-subsection"><h2>{failedQueue === 'dead-letter' ? 'Dead-letter jobs' : `Failed jobs · ${failedQueue}`}</h2><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Job</th><th>Source</th><th>Attempts</th><th>Reason</th><th>Finished</th><th aria-label="Actions" /></tr></thead><tbody>{failedJobs.length === 0 ? <tr><td colSpan={6}>No failed jobs.</td></tr> : failedJobs.map((job) => <tr key={job.id}><td>{job.id}</td><td>{job.sourceQueue ? `${job.sourceQueue} · ${job.sourceJobId}` : job.name}</td><td>{job.attemptsMade ?? 0}</td><td>{job.failedReason ?? 'Unknown failure'}</td><td>{job.failedAt ? new Date(job.failedAt).toLocaleString() : job.finishedOn ? new Date(job.finishedOn).toLocaleString() : '—'}</td><td><button className="admin-row-action" disabled={!permissions.includes('queues.retry')} onClick={() => void retry(failedQueue, job)}>Retry</button></td></tr>)}</tbody></table></div></section>}</section></div>{dialog}</>
}
