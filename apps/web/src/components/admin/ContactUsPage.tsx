...
import { CalendarDays, ChevronDown, Filter, LockKeyhole, MoreVertical, Send, ShieldCheck, UserRound } from 'lucide-react'
...
type Case = { id: string; subject: string; category: string; status: string; priority: string; assignedAdminId: string | null; slaDueAt: string | null; createdAt: string; requester: { id: string; name: string | null; email: string; plan: string; location: string | null; jobsCount: number; resumeExists: boolean; gmail: { connected: boolean; lastSyncedAt: string | null; hasError: boolean } }; messages: Message[] }
...
export function ContactUsPage() {
  ...
  return <div className="admin-page">
    ...
    <div className="support-workspace">
      ...
      <section className="support-thread">{selected ? <><div className="support-thread-top"><div><h2>{selected.subject}</h2><span>#{selected.id.slice(-6)} · {selected.category.replaceAll('_', ' ')}</span></div><button title="More case actions"><MoreVertical size={18} /></button></div><div className="support-controls"><button>{selected.status} <ChevronDown size={14} /></button><button><UserRound size={14} /> {selected.assignedAdminId ? 'Assigned' : 'Unassigned'} <ChevronDown size={14} /></button><span className="support-sla">SLA {relativeSla(selected.slaDueAt)}</span></div>
