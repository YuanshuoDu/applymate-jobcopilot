...
function UserDetail({ user, planChanges, loading, error, onReload }: { user: AdminUserRow | null; planChanges: Array<{ id: string; fromPlan: string; toPlan: string; createdAt: string }>; loading: boolean; error: string; onReload: () => void }) {
  ...
  async function changeState(next: 'active' | 'suspended') { if (!user || !window.confirm(next === 'suspended' ? t('adminUser.suspendConfirm') : t('adminUser.restoreConfirm'))) return; const reason = window.prompt(t('adminUser.reason'))?.trim() ?? ''; if (reason.length < 10) return; setBusy(true); const response = await fetch(`/api/admin/v1/users/${user.id}/account-state`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', Origin: window.location.origin, 'Idempotency-Key': key() }, body: JSON.stringify({ status: next, updatedAt: user.updatedAt, reason }) }); setBusy(false); if (response.ok) onReload(); }
  ...
  return <div style={{ maxWidth: 900, margin: '0 auto' }}><Link href="/admin/users" style={link}><ArrowLeft size={14} aria-hidden="true" /> {t('adminUser.backToUsers')}</Link><header style={{ ...headerStyle, marginTop: 18 }}><div><div style={eyebrow}>{t('adminUser.maskedAccount')}</div><h1 style={titleStyle}>{user.name || t('adminUser.unnamed')}</h1><p style={muted}>{user.email} · {user.region || t('adminUser.regionUnavailable')}</p></div><Status value={user.accountStatus} /></header>{error && <ErrorBox text={error} />}<section style={section}><h2 style={heading}>{t('adminUser.accountControls')}</h2><div style={detailGrid}><div><span style={muted}>{t('admin.plan')}</span><strong style={{ display: 'block', marginTop: 4 }}>{user.plan}</strong></div><div><span style={muted}>{t('adminUser.jobs')}</span><strong style={{ display: 'block', marginTop: 4 }}>{user.counts.jobs}</strong></div><div><span style={muted}>{t('adminUser.resumes')}</span><strong style={{ display: 'block', marginTop: 4 }}>{user.counts.resumes}</strong></div><div><span style={muted}>{t('adminUser.tasks')}</span><strong style={{ display: 'block', marginTop: 4 }}>{user.counts.applicationTasks}</strong></div></div><button type="button" disabled={busy} onClick={() => void changeState(user.accountStatus === 'suspended' ? 'active' : 'suspended')} style={user.accountStatus === 'suspended' ? primary : danger}>{user.accountStatus === 'suspended' ? <PlayCircle size={15} aria-hidden="true" /> : <PauseCircle size={15} aria-hidden="true" />}{user.accountStatus === 'suspended' ? t('adminUser.restore') : t('adminUser.suspend')}</button></section><section style={section}><h2 style={heading}>{t('adminUser.manualPlan')}</h2><div style={inlineForm}><select value={selectedPlan} onChange={event => setSelectedPlan(event.target.value)} style={input}><option value={user.plan}>{user.plan} ({t('adminUser.current')})</option>{availablePlans.map(item => <option key={item.plan} value={item.plan}>{item.name}</option>)}</select><input value={reason} onChange={event => setReason(event.target.value)} placeholder={t('adminUser.reason')} style={{ ...input, flex: 1 }} /><button type="button" disabled={busy || selectedPlan === user.plan || reason.trim().length < 10 || user.accountStatus === 'suspended'} onClick={() => void changePlan()} style={primary}>{t('adminUser.applyPlan')}</button></div>{user.accountStatus === 'suspended' && <p style={muted}>{t('adminUser.restoreBeforePlan')}</p>}</section><section style={section}><h2 style={heading}>{t('adminUser.featureOverride')}</h2><div style={overrideGrid}><select value={overrideKey} onChange={event => setOverrideKey(event.target.value)} style={input}>{['ai_credits', 'job_discovery', 'auto_apply', 'tailored_resume', 'cover_letter', 'gmail_tracking', 'api_access'].map(item => <option key={item} value={item}>{item}</option>)}</select><label style={checkLabel}><input type="checkbox" checked={overrideEnabled} onChange={event => setOverrideEnabled(event.target.checked)} /> {t('adminUser.enabled')}</label><input type="number" min="0" value={overrideLimit} onChange={event => setOverrideLimit(event.target.value)} placeholder={t('adminUser.limit')} style={input} /><input type="datetime-local" value={overrideExpiry} onChange={event => setOverrideExpiry(event.target.value)} style={input} /><input value={overrideReason} onChange={event => setOverrideReason(event.target.value)} placeholder={t('adminUser.reason')} style={{ ...input, gridColumn: 'span 2' }} /><button type="button" disabled={busy || overrideReason.trim().length < 10} onClick={() => void saveOverride()} style={primary}>{t('adminUser.saveOverride')}</button></div>{overrides.length > 0 && <ul style={{ margin: '14px 0 0', paddingLeft: 18 }}>{overrides.map(item => <li key={item.id} style={{ marginBottom: 6 }}>{item.featureKey}: {item.enabled ? t('adminUser.enabled').toLowerCase() : t('adminUser.disabled')}{item.limit === null ? '' : ` · ${t('adminUser.limitPrefix')} ${item.limit}`}{item.expiresAt ? ` · ${t('adminUser.untilPrefix')} ${new Date(item.expiresAt).toLocaleDateString()}` : ''}</li>)}</ul>}</section><section style={section}><h2 style={heading}>{t('adminUser.planHistory')}</h2>{planChanges.length === 0 ? <p style={muted}>{t('adminUser.noChanges')}</p> : <ul style={{ margin: 0, paddingLeft: 18 }}>{planChanges.map(change => <li key={change.id} style={{ marginBottom: 7 }}>{change.fromPlan} → {change.toPlan} <span style={muted}>({new Date(change.createdAt).toLocaleDateString()})</span></li>)}</ul>}</section></div>
}

function Status({ value }: { value: string }) { const { t } = useI18n(); return <span style={{ display: 'inline-block', padding: '3px 7px', borderRadius: 999, background: value === 'active' ? '#e7f6ef' : '#fff1e8', color: value === 'active' ? '#13734f' : '#a34b19', fontSize: 11, fontWeight: 700 }}>{value === 'active' ? t('admin.active') : t('admin.suspended')}</span> }
