...
const kindLabels: Record<PlanEntitlementKind, string> = { boolean: 'planEditor.enabled', limit: 'planEditor.limited', unlimited: 'planEditor.unlimited', text: 'planEditor.namedLevel' }
...
export function PlanEntitlementEditor({ values, disabled, onChange }: { values: readonly string[]; disabled?: boolean; onChange: (values: string[]) => void }) {
  ...
  function commit(next: PlanEntitlementDraft[]) {
    setDrafts(next)
    try { onChange(encodeEntitlements(next)); setError('') } catch (commitError) { setError(commitError instanceof Error ? commitError.message : t('planEditor.fixPermissions')) }
  }
  ...
  return <section style={{ marginTop: 22 }}>
    <div style={sectionHeader}><div><h3 style={heading}>{t('planEditor.runtimeHeading')}</h3><p style={help}>{t('planEditor.runtimeHelp')}</p></div><button type="button" disabled={disabled} onClick={add} style={secondary}><Plus size={14} aria-hidden="true" /> {t('planEditor.addPermission')}</button></div>
    <div style={list}>{drafts.map((draft, index) => {
      ...
      return <div key={draft.id} style={card}>
        <div style={row}><select aria-label={`${t('planEditor.permission')} ${index + 1}`} value={definition ? draft.key : CUSTOM_KEY} disabled={disabled} onChange={event => changeKey(index, event.target.value)} style={input}>{PLAN_ENTITLEMENT_DEFINITIONS.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}<option value={CUSTOM_KEY}>{t('planEditor.customPermission')}</option></select><button type="button" aria-label={`${t('planEditor.removePermission')} ${index + 1}`} disabled={disabled} onClick={() => commit(drafts.filter((_, draftIndex) => draftIndex !== index))} style={icon}><Trash2 size={14} aria-hidden="true" /></button></div>
        {!definition && <input aria-label={`${t('planEditor.customPermissionKey')} ${index + 1}`} value={draft.key} disabled={disabled} onChange={event => patch(index, { key: event.target.value })} placeholder="lowercase_permission_key" style={{ ...input, marginTop: 8 }} />}
        <div style={valueRow}><select aria-label={`${t('planEditor.permissionType')} ${index + 1}`} value={draft.kind} disabled={disabled || kindOptions.length === 1} onChange={event => patch(index, { kind: event.target.value as PlanEntitlementKind, value: event.target.value === 'limit' ? 0 : definition?.options?.[0]?.value ?? '' })} style={smallInput}>{kindOptions.map(kind => <option key={kind} value={kind}>{t(kindLabels[kind])}</option>)}</select>{draft.kind === 'limit' && <><input aria-label={`${t('planEditor.limitFor')} ${index + 1}`} type="number" min="0" step="1" value={String(draft.value)} disabled={disabled} onChange={event => patch(index, { value: Number(event.target.value) })} style={smallInput} /><select aria-label={`${t('planEditor.limitPeriodFor')} ${index + 1}`} value={draft.unit} disabled={disabled} onChange={event => patch(index, { unit: event.target.value as 'month' | '' })} style={smallInput}><option value="">{t('planEditor.total')}</option><option value="month">{t('planEditor.perMonth')}</option></select></>}{draft.kind === 'text' && (definition?.options ? <select aria-label={`${t('planEditor.valueFor')} ${index + 1}`} value={String(draft.value)} disabled={disabled} onChange={event => patch(index, { value: event.target.value })} style={smallInput}>{definition.options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : <input aria-label={`${t('planEditor.valueFor')} ${index + 1}`} value={String(draft.value)} disabled={disabled} onChange={event => patch(index, { value: event.target.value })} placeholder={t('planEditor.permissionLevel')} style={smallInput} />)}{draft.kind === 'boolean' && <span style={enabled}>{t('planEditor.enabledForPlan')}</span>}{draft.kind === 'unlimited' && <span style={enabled}>{t('planEditor.noNumericLimit')}</span>}</div>
      </div>
    })}</div>
    {drafts.length === 0 && <p style={empty}>{t('planEditor.noRuntimePermissions')}</p>}
