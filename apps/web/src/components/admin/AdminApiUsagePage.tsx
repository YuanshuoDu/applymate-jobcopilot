...
type UserSummary = { userId: string; category: 'job' | 'ai' | 'external'; calls: number; jobs: number; tokens: number; bytes: number; cost: number; errors: number; avgLatency: number; lastEventAt: string | null }
...
type ExternalProvider = { key: string; label: string; category: string; access: string; billing: string; telemetry: string; calls: number; inputBytes: number; outputBytes: number; cost: number; costKnown?: boolean; currency: string | null; errors: number; avgLatency: number; lastEventAt: string | null; source: string; period: string | null; sampledAt: string | null; alertThresholdUsd: number | null; maxBudgetUsd: number | null; alertTriggered: boolean; metrics?: ExternalMetric[]; operations: Array<{ operation: string; credentialSource: string; calls: number; inputBytes: number; outputBytes: number; cost: number; errors: number }> }
...
const formatCost = (value: number, currency: string | null = 'USD') => `${currency && currency !== 'USD' ? `${currency} ` : '$'}${value.toFixed(4)}`
...
function UserUsageSection({ category, users, details, selectedUserId, onSelect, t }: { category: 'job' | 'ai' | 'external'; users: UserSummary[]; details: UserDetail[]; selectedUserId: string; onSelect: (value: string) => void; t: (key: string) => string }) {
  return <section className="api-usage-section api-user-usage"><div className="api-usage-section-header"><div><h2>{t('apiUsage.users')}</h2><p>{t('apiUsage.userHelp')}</p></div><label>{t('apiUsage.selectUser')}<select value={selectedUserId} onChange={event => onSelect(event.target.value)}><option value="">{t('apiUsage.allUsers')}</option>{users.map(user => <option key={user.userId} value={user.userId}>{user.userId}</option>)}</select></label></div>
    <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>{t('apiUsage.userId')}</th><th>{t('apiUsage.calls')}</th><th>{category === 'job' ? t('apiUsage.jobsReturned') : category === 'ai' ? t('apiUsage.tokens') : t('apiUsage.data')}</th><th>{t('apiUsage.cost')}</th><th>{t('apiUsage.errors')}</th><th>{t('apiUsage.lastEvent')}</th></tr></thead><tbody>{users.length ? users.map(user => <tr key={user.userId} data-selected={selectedUserId === user.userId}><td><button className="admin-row-action" type="button" onClick={() => onSelect(user.userId)}>{user.userId}</button></td><td>{format(user.calls)}</td><td>{format(category === 'job' ? user.jobs : category === 'ai' ? user.tokens : user.bytes)}</td><td>{category !== 'job' ? `$${user.cost.toFixed(6)}` : '—'}</td><td>{format(user.errors)}</td><td>{freshness(user.lastEventAt, t)}</td></tr>) : <tr><td colSpan={6}>{t('apiUsage.noUserData')}</td></tr>}</tbody></table></div>
    {selectedUserId && <><h3 className="api-user-detail-title">{t('apiUsage.userDetail')} · {selectedUserId}</h3><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>{t('apiUsage.provider')}</th><th>{t('apiUsage.operationModel')}</th><th>{t('apiUsage.feature')}</th><th>{t('apiUsage.runtime')}</th><th>{t('apiUsage.owner')}</th><th>{t('apiUsage.calls')}</th><th>{category === 'job' ? t('apiUsage.jobsReturned') : category === 'ai' ? t('apiUsage.tokens') : t('apiUsage.data')}</th><th>{t('apiUsage.errors')}</th><th>{t('apiUsage.latency')}</th></tr></thead><tbody>{details.length ? details.map((row, index) => <tr key={`${row.provider}-${row.operationModel}-${row.runtime}-${index}`}><td>{row.provider}</td><td>{row.operationModel}</td><td>{row.featureKey || '—'}</td><td>{t(`apiUsage.runtime.${row.runtime}`)}</td><td>{t(`apiUsage.owner.${row.credentialSource}`)}</td><td>{format(row.calls)}</td><td>{format(category === 'job' ? row.jobs : category === 'ai' ? row.tokens : row.bytes)}</td><td>{format(row.errors)}</td><td>{format(row.avgLatency)} ms</td></tr>) : <tr><td colSpan={9}>{t('apiUsage.noUserData')}</td></tr>}</tbody></table></div></>}
  </section>
}
