...
type Payload = { generatedAt: string; days: number; provider: string | null; catalog: { job: ProviderOption[]; ai: ProviderOption[] }; job: { summary: { calls: number; jobs: number; errors: number; errorRate: number }; providers: JobProvider[] }; ai: { summary: { calls: number; tokens: number; costUsd: number; errors: number; errorRate: number }; providers: AiProvider[] }; quotas: Quota[]; optimization?: { cacheHits: number; singleflightHits: number; providerSkips: number; shadowJobs: number; shadowNetNewJobs: number; shadowValidApplyUrls: number; shadowCompleteDescriptions: number }; trend: Array<{ day: string; category: string; calls: number; units: number; cost: number }> }
...
export function AdminApiUsagePage({ canUpdateJob, canUpdateAi }: { canUpdateJob: boolean; canUpdateAi: boolean }) {
  ...
  return <div className="admin-page">
    ...
    <main className="api-usage-layout">
      ...
      <section className="api-usage-section"><h2>{t('apiUsage.breakdown')}</h2><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>{t('apiUsage.provider')}</th><th>{t('apiUsage.operationModel')}</th><th>{t('apiUsage.owner')}</th><th>{t('apiUsage.calls')}</th><th>{category === 'job' ? t('apiUsage.jobsReturned') : t('apiUsage.tokens')}</th><th>{t('apiUsage.errors')}</th><th>{t('apiUsage.latency')}</th><th>{t('apiUsage.lastEvent')}</th></tr></thead><tbody>{category === 'job' ? data?.job.providers.map(provider => <tr key={provider.key}><td>{provider.label}</td><td>{provider.operations.map(row => row.operation).join(', ') || '—'}{provider.maxJobsPerResponse ? ` · ${t('apiUsage.maxJobs')} ${provider.maxJobsPerResponse}` : ''}{provider.fullJobData ? ` · ${t('apiUsage.fullData')}` : ''}</td><td>{provider.operations.map(row => t(`apiUsage.owner.${row.credentialSource}`)).filter((v, i, a) => a.indexOf(v) === i).join(', ') || t(`apiUsage.owner.${provider.access === 'public' ? 'public' : 'platform'}`)}</td><td>{format(provider.calls)}</td><td>{format(provider.jobs)}</td><td>{format(provider.errors)}</td><td>{format(provider.avgLatency)} ms</td><td>{freshness(provider.lastEventAt, t)}</td></tr>) : data?.ai.providers.map(provider => <tr key={`${provider.provider}-${provider.model}-${provider.credentialSource}`}><td>{provider.provider}</td><td>{provider.model}</td><td>{t(`apiUsage.owner.${provider.credentialSource}`)}</td><td>{format(provider.calls)}</td><td>{format(provider.inputTokens + provider.outputTokens)}</td><td>{format(provider.errors)}</td><td>{format(provider.avgLatency)} ms</td><td>{freshness(provider.lastEventAt, t)}</td></tr>)}</tbody></table></div></section>
    </main>
  </div>
}
