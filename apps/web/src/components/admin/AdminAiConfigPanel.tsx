...
export function AdminAiConfigPanel({ canUpdate }: { canUpdate: boolean }) {
  ...
  async function load() {
    const response = await fetch('/api/admin/v1/ai/config', { cache: 'no-store' })
    const payload = await response.json().catch(() => null) as Config | { error?: string } | null
    if (!response.ok) { setNotice((payload as { error?: string } | null)?.error ?? 'Unable to load AI configuration.'); return }
