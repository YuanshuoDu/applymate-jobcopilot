...
export function AdminAtsControls({ permissions }: { permissions: readonly string[] }) {
  const { t } = useI18n()
  const stateLabel = (state: string) => state === 'paused' ? t('adminAts.state.paused') : state === 'active' ? t('adminAts.state.active') : t('adminAts.state.pending')
  const propagationLabel = (value: string) => value === 'acknowledged' ? t('adminAts.propagation.acknowledged') : value === 'pending' ? t('adminAts.propagation.pending') : value === 'not-configured' ? t('adminAts.propagation.notConfigured') : value
  const auditActionLabel = (value: string) => value === 'ats.pause' ? t('adminAts.action.pause') : value === 'ats.resumed' ? t('adminAts.action.resume') : value === 'ats.policy_updated' ? t('adminAts.action.policyUpdated') : t('adminAts.action.other')
  const auditOutcomeLabel = (value: string) => value === 'success' ? t('adminAts.outcome.success') : value === 'failed' ? t('adminAts.outcome.failed') : value === 'denied' ? t('adminAts.outcome.denied') : t('adminAts.outcome.other')
  ...
  async function request(url: string, body: Record<string, unknown>) {
    ...
    setNotice(response.ok ? `${t('adminAts.sourceState')}: ${stateLabel(payload?.state ?? 'pending')}.` : t('adminAts.operationFailed'))
    return response.ok
  }
