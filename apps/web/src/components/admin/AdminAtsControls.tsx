...
type Policy = { state: string; enabled: boolean; rolloutPercent: number; globalRpsLimit: number; perTenantRpsLimit: number; maxRetries: number; backoffBaseMs: number; allowAutoApply: boolean; version: number; lastAcknowledgedVersion: number | null }
...
export function AdminAtsControls({ permissions }: { permissions: readonly string[] }) {
  ...
  async function save(source: Source) {
    ...
    const response = await fetch(`/api/admin/v1/ats/${source.sourceKey}/policy`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ ...policy, version: policy.version, reason }) })
