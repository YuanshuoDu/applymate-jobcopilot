...
function CreateProviderForm({ onCreated }: { onCreated: () => Promise<void> }) {
  ...
  return <div style={createGrid}><input value={key} onChange={event => setKey(event.target.value)} placeholder={t('ai.providerKey')} style={input} /><input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder={t('ai.displayName')} style={input} /><input value={apiBase} onChange={event => setApiBase(event.target.value)} placeholder={t('ai.httpsApiBase')} style={input} /><input value={secretRef} onChange={event => setSecretRef(event.target.value)} placeholder={t('ai.secretReference')} style={input} /><input value={reason} onChange={event => setReason(event.target.value)} placeholder={t('ai.reasonPlaceholder')} style={input} /><button type="button" disabled={busy || reason.trim().length < 10} onClick={() => void create()} style={primary}><Save size={14} aria-hidden="true" /> {t('ai.create')}</button></div>
}
