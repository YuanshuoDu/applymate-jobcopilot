...
export function AIPage() {
  ...
  return <div style={{ maxWidth: 1180, margin: '0 auto' }}><header style={headerStyle}><div><div style={eyebrow}>Platform operations</div><h1 style={titleStyle}>Platform AI</h1><p style={muted}>Manage provider metadata, model availability and feature routing. Secret values stay in the environment.</p></div><button type="button" title="Refresh AI configuration" onClick={() => void load()} style={iconButton}><RefreshCw size={16} aria-hidden="true" /></button></header>{error && <ErrorBox text={error} />}{loading ? <p style={muted}>Loading AI configuration…</p> : <><section style={section}><div style={sectionHeader}><h2 style={heading}>Providers</h2><button type="button" title="Create provider" onClick={() => setShowCreateProvider(value => !value)} style={secondary}><Plus size={14} aria-hidden="true" /> Create provider</button></div>{showCreateProvider && <CreateProviderForm onCreated={async () => { setShowCreateProvider(false); await load() }} />}{providers.length > 0 && <div style={providerGrid}>{providers.map(provider => <ProviderCard key={provider.id} provider={provider} onSaved={load} />)}</div>}{providers.length === 0 && <p style={muted}>No providers have been seeded.</p>}</section><section style={section}><h2 style={heading}>Feature routing</h2><div style={{ display: 'grid', gap: 8 }}>{routes.map(route => <RouteRow key={route.id} route={route} models={models} onSaved={load} />)}</div>{routes.length === 0 && <p style={muted}>No feature routes have been seeded.</p>}</section></>}</div>
}
...
function CreateProviderForm({ onCreated }: { onCreated: () => Promise<void> }) {
  ...
  return <div style={createGrid}><input value={key} onChange={event => setKey(event.target.value)} placeholder="Provider key" style={input} /><input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Display name" style={input} /><input value={apiBase} onChange={event => setApiBase(event.target.value)} placeholder="HTTPS API base" style={input} /><input value={secretRef} onChange={event => setSecretRef(event.target.value)} placeholder="Secret reference (optional)" style={input} /><input value={reason} onChange={event => setReason(event.target.value)} placeholder="Reason (10+ characters)" style={input} /><button type="button" disabled={busy || reason.trim().length < 10} onClick={() => void create()} style={primary}><Save size={14} aria-hidden="true" /> Create</button></div>
}
...
function ProviderCard({ provider, onSaved }: { provider: AiProviderDto; onSaved: () => Promise<void> }) {
  ...
  async function test() { const testReason = window.prompt('Reason (10-500 characters)')?.trim() ?? ''; if (testReason.length < 10) return; setBusy(true); try { const response = await fetch(`/api/admin/v1/ai/providers/${provider.id}/test`, { method: 'POST', headers: headers(), body: JSON.stringify({ model: provider.models.find(model => model.active)?.model, reason: testReason }) }); const body = await response.json() as { ok?: boolean; errorClass?: string; error?: string }; if (!response.ok || !body.ok) window.alert(body.errorClass ?? body.error ?? 'Provider test failed'); else window.alert('Provider test passed') } finally { setBusy(false) } }
  return <article style={card}><div style={cardHeader}><div><h3 style={{ margin: 0, fontSize: 16 }}>{provider.displayName}</h3><span style={muted}>{provider.key} · v{provider.version}</span></div><span style={{ ...status, background: providerHealthLabel(provider) === 'Ready' ? '#e7f6ef' : '#fff1e8', color: providerHealthLabel(provider) === 'Ready' ? '#13734f' : '#a34b19' }}>{providerHealthLabel(provider)}</span></div><div style={metaGrid}><div><span style={muted}>Secret reference</span><strong style={value}>{provider.secretRef || 'Not configured'}</strong></div><div><span style={muted}>API base</span><strong style={value}>{provider.apiBase}</strong></div></div><label style={label}>Display name<input value={draft.displayName} onChange={event => setDraft({ ...draft, displayName: event.target.value })} style={input} /></label><label style={label}>Audit reason<input value={reason} onChange={event => setReason(event.target.value)} placeholder="Reason (10+ characters)" style={input} /></label><div style={buttonRow}><button type="button" disabled={busy || reason.trim().length < 10} onClick={() => void save()} style={primary}><Save size={14} aria-hidden="true" /> Save</button><button type="button" disabled={busy || !provider.credentialConfigured} onClick={() => void test()} style={secondary}><TestTube2 size={14} aria-hidden="true" /> Test connection</button><button type="button" disabled={busy || reason.trim().length < 10} onClick={() => void toggleEnabled()} style={secondary}>{draft.enabled ? <CheckCircle2 size={14} aria-hidden="true" /> : <XCircle size={14} aria-hidden="true" />}{draft.enabled ? 'Enabled' : 'Disabled'}</button></div><div style={modelList}><div style={modelHeader}><strong style={{ fontSize: 12 }}>Models</strong><button type="button" title="Create model" onClick={() => setShowCreateModel(value => !value)} style={smallButton}><Plus size={13} aria-hidden="true" /> Add</button></div>{showCreateModel && <CreateModelForm providerId={provider.id} onCreated={async () => { setShowCreateModel(false); await onSaved() }} />}{provider.models.map(model => <ModelRow key={model.id} provider={provider} model={model} onSaved={onSaved} />)}</div></article>
}
...
function CreateModelForm({ providerId, onCreated }: { providerId: string; onCreated: () => Promise<void> }) {
  ...
  return <div style={createGrid}><input value={model} onChange={event => setModel(event.target.value)} placeholder="Model identifier" style={input} /><input value={label} onChange={event => setLabel(event.target.value)} placeholder="Label" style={input} /><select value={tier} onChange={event => setTier(event.target.value)} style={input}><option value="fast">Fast</option><option value="standard">Standard</option><option value="premium">Premium</option></select><input type="number" min="1" value={contextK} onChange={event => setContextK(event.target.value)} placeholder="Context K" style={input} /><input value={reason} onChange={event => setReason(event.target.value)} placeholder="Reason (10+ characters)" style={input} /><button type="button" disabled={busy || reason.trim().length < 10} onClick={() => void create()} style={primary}><Save size={14} aria-hidden="true" /> Create</button></div>
}

function ModelRow({ provider, model, onSaved }: { provider: AiProviderDto; model: AiModelDto; onSaved: () => Promise<void> }) {
  async function toggle() { const reason = window.prompt('Reason (10-500 characters)')?.trim() ?? ''; if (reason.length < 10) return; const response = await fetch(`/api/admin/v1/ai/providers/${provider.id}/models/${model.id}`, { method: 'PATCH', headers: headers(), body: JSON.stringify({ model: model.model, label: model.label, description: model.description, tier: model.tier, priceIn: model.priceIn, priceOut: model.priceOut, contextK: model.contextK, active: !model.active, reason }) }); if (response.ok) await onSaved() }
  return <div style={modelRow}><span>{model.label}<small style={muted}>{model.model}</small></span><span style={muted}>{model.contextK}K · ${model.priceIn}/{model.priceOut}</span><button type="button" title={model.active ? 'Disable model' : 'Enable model'} onClick={() => void toggle()} style={smallButton}>{model.active ? 'Active' : 'Inactive'}</button></div>
}
