...
import { Bot, CheckCircle2, RefreshCw, Save, TestTube2, XCircle } from 'lucide-react'
...
function ProviderCard({ provider, onSaved }: { provider: AiProviderDto; onSaved: () => Promise<void> }) {
  ...
  return <article style={card}><div style={cardHeader}><div><h3 style={{ margin: 0, fontSize: 16 }}>{provider.displayName}</h3><span style={muted}>{provider.key} · v{provider.version}</span></div><span style={{ ...status, background: providerHealthLabel(provider) === 'Ready' ? '#e7f6ef' : '#fff1e8', color: providerHealthLabel(provider) === 'Ready' ? '#13734f' : '#a34b19' }}>{providerHealthLabel(provider)}</span></div><div style={metaGrid}><div><span style={muted}>Secret reference</span><strong style={value}>{provider.secretRef || 'Not configured'}</strong></div><div><span style={muted}>API base</span><strong style={value}>{provider.apiBase}</strong></div></div><label style={label}>Display name<input value={draft.displayName} onChange={event => setDraft({ ...draft, displayName: event.target.value })} style={input} /></label><label style={label}>Audit reason<input value={reason} onChange={event => setReason(event.target.value)} placeholder="Reason (10+ characters)" style={input} /></label><div style={buttonRow}><button type="button" disabled={busy || reason.trim().length < 10} onClick={() => void save()} style={primary}><Save size={14} aria-hidden="true" /> Save</button><button type="button" disabled={busy || !provider.credentialConfigured} onClick={() => void test()} style={secondary}><TestTube2 size={14} aria-hidden="true" /> Test connection</button><button type="button" disabled={busy || reason.trim().length < 10} onClick={() => void save()} style={secondary}>{draft.enabled ? <CheckCircle2 size={14} aria-hidden="true" /> : <XCircle size={14} aria-hidden="true" />}{draft.enabled ? 'Enabled' : 'Disabled'}</button></div><div style={modelList}><strong style={{ fontSize: 12 }}>Models</strong>{provider.models.map(model => <ModelRow key={model.id} provider={provider} model={model} onSaved={onSaved} />)}</div></article>
}
