...
import type { EntitlementKind, EntitlementValue, PlanCatalogDto, PlanKey } from '@/lib/admin/plans'
...
export function PlansPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const planBody = await plansResponse.json() as { items?: PlanCatalogDto[]; error?: string }
      ...
      setPlans(planBody.items ?? []); setTransitions(transitionBody.items ?? [])
  ...
  return <div style={{ maxWidth: 1180, margin: '0 auto' }}>
    ...
    {loading ? <p style={muted}>Loading plans…</p> : <>
      <section style={section}><div style={sectionHeader}><h2 style={heading}>Plan catalogue</h2><button type="button" title="Create plan" style={secondary} onClick={() => setShowCreate(value => !value)}><Plus size={15} aria-hidden="true" /> Create</button></div>{showCreate && <CreatePlanForm onCreated={async () => { setShowCreate(false); await load() }} />}{plans.length > 0 && <div style={planGrid}>{plans.map(plan => <PlanCard key={plan.id} plan={plan} onSaved={load} />)}</div>}{plans.length === 0 && <p style={muted}>No plans have been seeded yet.</p>}</section>
...
function CreatePlanForm({ onCreated }: { onCreated: () => Promise<void> }) {
  const [plan, setPlan] = useState<PlanKey>('free'); const [name, setName] = useState(''); const [monthly, setMonthly] = useState('0'); const [yearly, setYearly] = useState('0'); const [reason, setReason] = useState('')
  const [busy, setBusy] = useState(false)
  async function create() {
    if (!name.trim() || reason.trim().length < 10) return
    setBusy(true)
    try { const response = await fetch('/api/admin/v1/plans', { method: 'POST', headers: headers(), body: JSON.stringify({ plan, name, monthlyPriceCents: Number(monthly), yearlyPriceCents: Number(yearly), currency: 'EUR', reason }) }); if (!response.ok) throw new Error('Plan creation failed'); await onCreated() } finally { setBusy(false) }
  }
  return <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 8, padding: '0 0 16px', marginBottom: 16, borderBottom: '1px solid #e5ebf1' }}><select value={plan} onChange={event => setPlan(event.target.value as PlanKey)} style={textInput}>{(['free', 'pro', 'enterprise'] as PlanKey[]).map(item => <option key={item} value={item}>{item}</option>)}</select><input value={name} onChange={event => setName(event.target.value)} placeholder="Plan name" style={textInput} /><input type="number" min="0" value={monthly} onChange={event => setMonthly(event.target.value)} placeholder="Monthly cents" style={textInput} /><input type="number" min="0" value={yearly} onChange={event => setYearly(event.target.value)} placeholder="Yearly cents" style={textInput} /><input value={reason} onChange={event => setReason(event.target.value)} placeholder="Reason (10+ chars)" style={textInput} /><button type="button" disabled={busy || !name.trim() || reason.trim().length < 10} onClick={() => void create()} style={primary}><Save size={14} aria-hidden="true" /> Publish</button></div>
}
...
function PlanCard({ plan, onSaved }: { plan: PlanCatalogDto; onSaved: () => Promise<void> }) {
  ...
  async function save() {
    ...
    setBusy(true)
    try {
      const response = await fetch(`/api/admin/v1/plans/${plan.plan}`, { method: 'PATCH', headers: headers(), body: JSON.stringify({ ...draft, version: plan.version, reason }) })
