...
export function AdminUsersPage({ canExport = false, permissions = [] }: { canExport?: boolean; permissions?: readonly string[] }) {
  ...
  return <><div className="admin-list-toolbar"><span>Exports contain deterministic hashes only; no email, resume, or document content.</span>{canExport && <a className="admin-secondary" href="/api/admin/v1/export?resource=users" download>Download anonymized CSV</a>}</div><AdminDataTable title="Users" subtitle="Masked account metadata and operational status" endpoint="/api/admin/v1/users" searchable columns={[
    { label: 'User', value: (row) => <Link prefetch={false} className="admin-table-link" href={`/admin/users/${row.id}`}>{String(row.name ?? 'Unnamed')} · {String(row.email ?? '')}</Link> }, { label: 'Plan', value: values.text('plan') },
...
export function AdminApplicationsPage({ permissions = [] }: { permissions?: readonly string[] }) {
  ...
  return <><AdminDataTable title="Applications" subtitle="Safe outcome metadata without job content or candidate documents" endpoint="/api/admin/v1/applications" columns={[
    { label: 'ID', sortKey: 'createdAt', value: row => <Link prefetch={false} className="admin-table-link" href={`/admin/applications/${row.id}`}>{String(row.id)}</Link> }, { label: 'Status', sortKey: 'status', value: values.text('status') }, { label: 'ATS', value: values.text('atsType') }, { label: 'Flow', value: values.text('flowUsed') },
