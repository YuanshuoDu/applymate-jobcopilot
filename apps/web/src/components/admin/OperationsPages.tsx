...
export function AdminUsersPage({ canExport = false, permissions = [] }: { canExport?: boolean; permissions?: readonly string[] }) {
  ...
  return <><div className="admin-list-toolbar"><span>{t('ops.exportNotice')}</span></div><AdminDataTable title={t('ops.usersTitle')} subtitle={t('ops.usersSubtitle')} endpoint="/api/admin/v1/users" searchable searchLabel="Search users" searchPlaceholder="Search name or email" columns={[
