...
export function useAdminPrompt() {
  ...
  const dialog = config ? <div className="security-dialog-backdrop"><form className="security-card security-dialog" role="dialog" aria-modal="true" onSubmit={(event) => { event.preventDefault(); close(value.trim()) }}><h2>{config.title}</h2>{config.description && <p>{config.description}</p>}<label>{config.label}{config.kind === 'datetime' ? <input type="datetime-local" required value={value} onChange={(event) => setValue(event.target.value)} autoFocus /> : <textarea required minLength={config.kind === 'reason' || !config.kind ? 10 : undefined} maxLength={config.kind === 'reason' ? 500 : 5000} rows={config.kind === 'text' ? 3 : 4} value={value} onChange={(event) => setValue(event.target.value)} autoFocus />}</label><div className="admin-inline-actions"><button className="admin-row-action" type="button" onClick={() => close(null)}>Cancel</button><button className="broadcast-primary" type="submit" disabled={!value.trim() || (config.kind === 'reason' && value.trim().length < 10)}>{config.submitLabel ?? 'Continue'}</button></div></form></div> : null
