...
export function AdminAiUsageTrends() {
  ...
  useEffect(() => {
    ...
    void fetchWithTimeout('/api/admin/v1/ai/usage?days=30', { cache: 'no-store' }).then(async response => {
      ...
      if (!response.ok) throw new Error(payload && 'error' in payload ? payload.error ?? t('aiUsage.loadFailed') : t('aiUsage.loadFailed'))
      return payload as Usage
    }).then(payload => { if (active) setUsage(payload) }).catch(loadError => { if (active) setError(loadError instanceof Error ? loadError.message : t('aiUsage.loadFailed')) })
    return () => { active = false }
  }, [])
  if (!usage) return <section className="admin-ai-config"><p role="status">{error || t('aiUsage.loading')}</p></section>
  const maxCalls = Math.max(...usage.trend.map(item => item.calls), 1)
  return <section className="admin-ai-config"><div className="admin-controls-title"><div><h2>{t('aiUsage.title')}</h2><p>{t('aiUsage.description')}</p></div></div><div className="admin-metric-grid"><div className="admin-metric"><span>{t('aiUsage.calls')}</span><strong>{usage.summary.calls}</strong></div><div className="admin-metric"><span>{t('aiUsage.estimatedCost')}</span><strong>${usage.summary.costUsd.toFixed(4)}</strong></div><div className="admin-metric"><span>{t('aiUsage.errorRate')}</span><strong>{usage.summary.errorRate}%</strong></div></div><div className="admin-trend-grid">{usage.trend.length === 0 ? <p>{t('aiUsage.empty')}</p> : usage.trend.map(item => <div className="admin-trend-row" key={item.day}><strong>{new Date(item.day).toLocaleDateString()}</strong><span className="admin-trend-bar"><i style={{ width: `${Math.max(3, item.calls / maxCalls * 100)}%` }} /></span><span>{item.calls} {t('aiUsage.calls')} · {item.avgLatency}ms · ${item.cost.toFixed(4)}</span></div>)}</div><div className="admin-table-wrap" style={{ marginTop: 18 }}><table className="admin-table"><thead><tr><th>{t('aiUsage.providerModel')}</th><th>{t('aiUsage.calls')}</th><th>{t('aiUsage.errors')}</th><th>{t('aiUsage.cost')}</th><th>{t('aiUsage.latency')}</th></tr></thead><tbody>{usage.providers.map(item => <tr key={`${item.provider}:${item.model}`}><td>{item.provider} · {item.model}</td><td>{item.calls}</td><td>{item.errors}</td><td>${item.cost.toFixed(4)}</td><td>{item.avgLatency}ms</td></tr>)}</tbody></table></div></section>
}
