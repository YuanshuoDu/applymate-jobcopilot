...
export function AdminAiUsageTrends() {
  ...
  useEffect(() => { void fetch('/api/admin/v1/ai/usage?days=30', { cache: 'no-store' }).then(response => response.ok ? response.json() : null).then(payload => setUsage(payload)).catch(() => undefined) }, [])
  if (!usage) return <section className="admin-ai-config"><p>Loading AI usage trends...</p></section>
