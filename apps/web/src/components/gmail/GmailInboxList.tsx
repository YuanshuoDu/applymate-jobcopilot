...
export function GmailInboxList({ emails, selectedId, onSelect }: GmailInboxListProps) {
  ...
  return <section style={{
    ...
    {emails.length === 0 ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>No emails in this category.</div>
      : emails.map(email => <EmailRow key={email.id} email={email} selected={selectedId === email.id} onSelect={onSelect} />)}
  </section>
}

function EmailRow({ email, selected, onSelect }: { email: GmailEmail; selected: boolean; onSelect: (email: GmailEmail) => void }) {
  ...
  return <button type="button" onClick={() => onSelect(email)} style={{
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, minWidth: 0 }}>
        {!email.read && <span aria-label="Unread" style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--primary)', flexShrink: 0 }} />}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
        {email.starred && <span aria-label="Starred" style={{ color: '#F59E0B', fontSize: 11 }}>★</span>}
