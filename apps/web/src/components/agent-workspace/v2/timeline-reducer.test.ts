...
describe('timeline reducer', () => {
  ...
  it('ignores retired plan projection events without adding them to timeline state', () => {
    const state = createTimelineState('session-1')
    const next = timelineReducer(state, { type: 'event', event: {
      schemaVersion: 'agent-harness.v2', id: 'plan-command-1', sessionId: 'session-1', turnId: 'turn-1', itemId: null, taskId: 'task-1',
      type: 'plan.command', actor: 'orchestrator', sequence: '12', payload: { private: 'not projected' },
    } })

    expect(next).not.toBe(state)
    expect(next.lastEventId).toBe('plan-command-1')
    expect(next.lastSequence).toBe('12')
    expect(next.events).toEqual([])
    expect(next.fallbackItems).toEqual([])
    expect(timelineReducer(next, { type: 'event', event: {
      schemaVersion: 'agent-harness.v2', id: 'older-event', sessionId: 'session-1', turnId: 'turn-1', itemId: null, taskId: null,
      type: 'turn.started', actor: 'orchestrator', sequence: '11', payload: {},
    } })).toBe(next)
  })
