...
export async function POST(request: NextRequest) {
  ...
  if (!invitation || invitation.email !== email || invitation.status !== 'pending' || invitation.expiresAt <= new Date()) return NextResponse.json({ error: 'Invitation is invalid or expired' }, { status: 400 })
