...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  await recordAiUsage({ userId: actor.userId, featureKey: 'providerTest', provider: provider.key, model: 'models-catalog', credentialSource: 'platform', latencyMs: Date.now() - started, status: status === 'ok' ? 'success' : 'error', errorCode: status === 'ok' ? undefined : status, chargeBudget: false })
