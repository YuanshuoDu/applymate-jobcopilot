...
type AtsPauseResult = { state: 'paused' | 'pending_pause'; version: number; alreadyPaused?: boolean; requiresSecondApprover?: boolean }
...
export async function POST(request: NextRequest, context: { params: Promise<{ sourceKey: string }> }) {
  ...
  try {
    const result = await runAdminMutation<AtsPauseResult>({
      ...
      idempotencyKey: key,
      targetId: sourceKey,
      audit: (value) => ({ requestId: actor.requestId, actorRoleKey: actor.roleKey, targetType: 'ats_source', targetId: sourceKey, reason, after: { state: value.state, version: value.version, propagation: value.state === 'paused' ? 'pending' : 'acknowledged' }, outcome: 'success' }),
      mutate: async (tx) => {
        const policy = await tx.atsSourcePolicy.findUnique({ where: { sourceKey }, select: { state: true, pauseRequestedById: true, version: true } })
        if (policy?.state === 'paused') return { state: 'paused' as const, version: policy.version, alreadyPaused: true }
        if (!policy) {
          ...
          return { state: 'pending_pause' as const, version: created.version, requiresSecondApprover: true }
        }
        if (policy.state === 'pending_pause') {
          ...
          return { state: 'paused' as const, version: approved.version, alreadyPaused: false }
        }
        ...
        return { state: 'pending_pause' as const, version: requested.version, requiresSecondApprover: true }
      },
    })
    ...
    return NextResponse.json({ state: 'paused', version: result.value.version, propagation }, { headers: { 'Cache-Control': 'no-store' } })
  } catch (error) {
