...
export async function POST(request: NextRequest) {
  ...
  const result = await runAdminMutation({ actorUserId: actor.userId, action: 'break_glass.requested', idempotencyKey: key, audit: { requestId: actor.requestId, actorRoleKey: actor.roleKey, reason, outcome: 'success', after: { permission: input.permission, durationMinutes: input.durationMinutes } }, mutate: (tx) => tx.adminBreakGlassGrant.create({ data: { requesterId: actor.userId, permission: input.permission, reason, expiresAt: new Date(Date.now() + input.durationMinutes * 60_000) }, select: { id: true, permission: true, expiresAt: true } }) })
