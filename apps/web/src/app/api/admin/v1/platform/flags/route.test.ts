...
describe('POST /api/admin/v1/platform/flags', () => {
  ...
  it('creates a constrained draft with an audit event', async () => {
    ...
    mocks.create.mockResolvedValue({ id: 'flag-1', key: 'new_feature', environment: 'development', enabled: false, rolloutPercent: 0, targetPlans: [], targetUserIds: [], status: 'draft', version: 1, createdById: 'admin-1', approvedById: null, rollbackAt: null, updatedAt: new Date() })
    ...
    expect(mocks.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ createdById: 'admin-1', updatedById: 'admin-1', environment: 'development' }) }))
  })
})
