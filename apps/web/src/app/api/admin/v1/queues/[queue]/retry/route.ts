...
export async function POST(request: NextRequest, context: { params: Promise<{ queue: string }> }) {
  ...
  if ((!queues.includes(queue) && queue !== deadLetterQueue) || !jobId || jobId.length > 200 || reason.length < 10 || reason.length > 500 || !key) return NextResponse.json({ error: 'Invalid queue retry' }, { status: 400 })
  ...
  try {
    const command = await sendWorkerCommand({ requestId: actor.requestId, actorId: actor.userId, action: queue === deadLetterQueue ? 'retry_dead_letter_job' : 'retry_queue_job', reason, params: queue === deadLetterQueue ? { deadLetterId: jobId } : { queue, jobId } })
