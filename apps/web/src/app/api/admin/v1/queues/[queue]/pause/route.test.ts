...
describe('POST /api/admin/v1/queues/:queue/pause', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
    ...
    mocks.mutation.mockResolvedValue({ duplicate: false, value: { queue: 'apply-tasks' } })
    mocks.worker.mockResolvedValue({ receipt: 'receipt-1' })
    mocks.audit.mockResolvedValue(undefined)
  })

  it('delegates queue pause to the atomic admin mutation helper', async () => {
    const { POST } = await import('./route')
    const response = await POST(new Request('http://localhost/api/admin/v1/queues/apply-tasks/pause', { method: 'POST', headers: { 'idempotency-key': 'key' }, body: JSON.stringify({ reason: 'Pause apply workers during a controlled maintenance window' }) }) as never, { params: Promise.resolve({ queue: 'apply-tasks' }) })
    expect(response.status).toBe(200)
    expect(mocks.mutation).toHaveBeenCalledWith(expect.objectContaining({ action: 'queue.pause_requested', idempotencyKey: 'key', targetId: 'apply-tasks' }))
  })
  ...
  it('dispatches the Worker command only after the audit transaction commits', async () => {
    ...
    await expect(response.json()).resolves.toEqual({ queue: 'apply-tasks', receipt: 'worker-receipt' })
  })
  ...
  it('records a failed dispatch and does not claim that the queue changed', async () => {
    ...
    expect(mocks.audit).toHaveBeenCalledWith(expect.objectContaining({ action: 'queue.pause_failed', outcome: 'failed' }))
  })
})
