...
export async function GET(request: NextRequest, context: { params: Promise<{ queue: string }> }) {
  ...
  if (!queues.includes(queue)) return NextResponse.json({ error: 'Unsupported queue' }, { status: 400 })
  try {
    const result = await sendWorkerCommand({ requestId: actor.requestId, actorId: actor.userId, action: 'failed_queue_jobs', reason: 'View failed queue jobs for operational triage', params: { queue } })
