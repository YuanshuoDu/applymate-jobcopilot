...
export async function GET(request: Request) {
  ...
  try {
    const actor = await requireAdminActor('support_cases.read', request)
    const url = new URL(request.url)
    const status = parseSupportCaseStatus(url.searchParams.get('status'))
    const priority = parseSupportCasePriority(url.searchParams.get('priority'))
    ...
    const cases = await db.supportCase.findMany({
      where: { ...(status ? { status } : {}), ...(priority ? { priority } : {}), ...(assignedAdminId ? { assignedAdminId } : {}), ...(category ? { category } : {}) },
      orderBy: [{ priority: 'desc' }, { slaDueAt: 'asc' }, { updatedAt: 'desc' }], take: limit, select: CASE_SELECT,
    })
