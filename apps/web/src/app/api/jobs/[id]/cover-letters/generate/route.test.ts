...
vi.mock('@/lib/db', () => ({ db: {
  ...
  coverLetter: { findFirst: mocks.coverLetterFindFirst, create: mocks.coverLetterCreate, update: mocks.coverLetterUpdate },
} }))
...
describe('cover letter generation after an audit failure', () => {
  ...
  it('repairs an existing cover letter in place and preserves its supported wording', async () => {
    ...
    const response = await POST(new NextRequest('http://localhost/api/jobs/job_1/cover-letters/generate', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        resumeId: 'resume_1', preferProvidedResume: true, repairCoverLetterId: 'letter_1',
        auditFindings: [{ area: 'cover_letter', severity: 'critical', title: 'Unsupported metric', evidence: '50% is absent.', action: 'Remove the metric.' }],
      }),
    }) as never, { params: Promise.resolve({ id: 'job_1' }) })
    ...
    expect(mocks.coverLetterUpdate).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'letter_1' } }))
    expect(mocks.modelChat.mock.calls[0][0][1].content).toContain('EXISTING COVER LETTER')
    expect(mocks.modelChat.mock.calls[0][0][1].content).toContain('preserve all supported wording and structure')
  })
})
