...
describe('GET /api/github/oauth/callback', () => {
  beforeEach(() => {
    ...
    mocks.userFindUnique.mockReset().mockResolvedValue({ id: 'user_1', accountStatus: 'active', authVersion: 1 })
  ...
  it('exchanges the code against the configured callback and redirects to the configured origin', async () => {
    const state = await new SignJWT({ uid: 'user_1', authVersion: 1, returnTo: '/?page=settings&tab=accounts', nonce: 'nonce-1' })
  ...
  it('does not link a GitHub account after the auth version changes', async () => {
    mocks.userFindUnique.mockResolvedValue({ id: 'user_1', accountStatus: 'active', authVersion: 2 })
    const state = await new SignJWT({ uid: 'user_1', authVersion: 1, returnTo: '/?page=settings&tab=accounts', nonce: 'nonce-1' })
      .setProtectedHeader({ alg: 'HS256' })
      .setExpirationTime('10m')
      .sign(new TextEncoder().encode('test-secret-which-is-long-enough'))
    ...
    expect(new URL(response.headers.get('location') ?? '').searchParams.get('githubError')).toBe('session_expired')
    expect(fetchMock).not.toHaveBeenCalled()
    expect(mocks.accountUpsert).not.toHaveBeenCalled()
  })
})
