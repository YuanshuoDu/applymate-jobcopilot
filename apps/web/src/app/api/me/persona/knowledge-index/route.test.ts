...
describe('POST /api/me/persona/knowledge-index', () => {
  beforeEach(() => { mocks.requireAuth.mockReset(); mocks.sync.mockReset(); mocks.requireAuth.mockResolvedValue({ userId: 'user_1' }); mocks.sync.mockResolvedValue({ candidates: 3, indexed: 3, embedded: 3, semanticEnabled: true }) })
  ...
  it('builds only the authenticated user knowledge index without caching the response', async () => {
    ...
    await expect(response.json()).resolves.toEqual({ candidates: 3, indexed: 3, embedded: 3, semanticEnabled: true })
  })
})
