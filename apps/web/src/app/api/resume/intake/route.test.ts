...
describe('POST /api/resume/intake plan access', () => {
  beforeEach(() => {
    vi.resetModules()
  ...
  it('rejects resume intake when AI credits are disabled', async () => {
    const { POST } = await import('./route')
    const response = await POST(new Request('http://localhost/api/resume/intake', { method: 'POST' }) as never)

    expect(response.status).toBe(403)
    expect(mocks.checkRateLimit).not.toHaveBeenCalled()
  })

  it('rejects resume intake when monthly credits are exhausted', async () => {
