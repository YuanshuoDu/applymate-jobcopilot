...
## P8-116 candidate - deterministic Worker startup composition fixture for #495 AC5

Worker startup now calls `startProductionAgentRuntime` from `index.ts`. The helper creates the canonical Turn runtime, registers the real production bootstrap (Turn consumer and recovery, plus only the configured child/wait consumers), synchronously transfers bootstrap cleanup ownership to the existing post-bootstrap fence, and only then starts the `agent-runs` router. The integration fixture calls this same helper with the real `createCanonicalTurnRuntime` and `createProductionWorkerBootstrap`, deterministic model/tool/context/store adapters, and injected queue/recovery factories. It asserts the queue receives the canonical executor, consumer and recovery registration precede router startup, the configured fixture `ModelAdapter` and `ToolRegistry` argument validator execute through the real `ToolRouter`, a persisted `tool_result` is present as the next model step receives its observation, and the second model step completes. A missing enabled child executor fails before bootstrap handoff or router startup; existing bootstrap cases retain wait/child gate coverage.

Focused production-bootstrap validation passed **12/12** and Worker `tsc --noEmit --skipLibCheck` passed. The fixture uses no live database, PostgreSQL/RLS, Redis/BullMQ, provider, network, browser, or application submission; it proves deterministic startup assembly and the injected runtime path only. Process restart, cross-worker delivery, production configuration, deployment, and real provider/browser execution remain unverified; P8-116 remains a candidate.

## P8-117 candidate - refresh supervisor projection on child task lifecycle events

The shared timeline reducer now recognizes the authoritative `task.started`, `task.completed`, `task.interrupted`, and `task.failed` events and advances `lifecycleRevision` only when the event is scoped to a non-null task ID. The supervisor's existing revision-driven query refresh therefore reloads task metadata after child lifecycle changes. Diagnostic task events such as `task.no_progress` and `task.budget_exhausted` remain visible as fallback diagnostics without causing redundant supervisor queries; unscoped task lifecycle events remain known but do not trigger refresh.

Focused timeline reducer and supervisor panel validation passed **29/29** tests, and `git diff --check` passed. This closes the task-status refresh gap identified in the AC4 source audit, but does not establish authenticated real-page browser evidence, application-review-control interaction, live event delivery, or production behavior. Formal P0-P7 acceptance remains **1/8 (12.5%)**; P8-117 remains a candidate.

## P8-118 candidate - safe Plan Ledger task-graph projection and bootstrap dispatch recovery

The existing Workbench timeline now accepts persisted `plan.task_graph` snapshots and projects a bounded, sanitized view into the Plan Ledger as distinct plan-step nodes. The projection contains only run/event/node identifiers, node and dependency IDs, status, and optional phase/attempt; absent a safe label it uses a generic label. Plan steps remain separate from leased worker tasks, and an untouched node without a phase does not display an attempt count. Replay and live updates continue through the existing scoped timeline path; this adds no graph persistence or worker scheduling behavior.

The deterministic `production-bootstrap` fixture now creates a fresh canonical runtime and bootstrap with a separate worker identity over its fake SQL pool, recovers one persisted queued Turn dispatch into the bootstrap-owned queue, marks the outbox row published, then runs the recovered job. The restarted runtime uses the production default `loadCanonicalTurnState` path with `consumeWaitOutcomes` enabled: it reads the owner-fenced Turn, consumes the ready durable wait result, and places that wait outcome and both child results in the resumed model request before the Turn completes. Focused Web parser, Plan Ledger view/card, timeline reducer, and timeline route checks passed **48/48**; focused Worker `production-bootstrap` validation passed **12/12**. Main-thread browser fixture verification with `pnpm test:e2e e2e/agent-supervisor.spec.ts --reporter=line` passed **4/4** across desktop/mobile and English/Chinese. Web and Worker `tsc --noEmit --skipLibCheck` and `git diff --check` passed at this code snapshot.

These are deterministic fixture results only. The fake SQL pool and injected queues do not prove real PostgreSQL/RLS behavior, Redis/BullMQ delivery, recovery across actual process restarts or workers, or production gates being enabled. Authenticated browser behavior, live timeline delivery, and production behavior remain unverified. Formal P0-P7 acceptance remains **1/8 (12.5%)**; P8-118 remains a candidate.

## P8-119 candidate - canonical planner execution through production startup

The focused `production-bootstrap` fixture now starts `createCanonicalTurnRuntime` through `startProductionAgentRuntime` and the production bootstrap with server-owned planning and plan-execution gates enabled. A fake Harness model proposes a plan through the real `createPlanProposalTool`; the canonical runtime's default `createCanonicalPlanExecutionFactory` validates and dispatches its read-only `jobs.search` command through `ToolRouter`. The fixture verifies that the runtime supplies the scoped user/session/turn/root-task identity and generated command ID, persists TaskGraph `search` start/complete events plus `finish` start/wait events for `propose_completion`, and makes the persisted result observation (at most 8 KiB) with bounded evidence and generated identity available in the next model-step request.

The focused Worker `production-bootstrap` test passed **13/13**, Worker `tsc` passed, and `git diff --check` passed. This is fixture evidence only. It does not cover the completion-verifier rejection branch or establish live PostgreSQL/RLS, provider, Redis/BullMQ, process-restart, or staging behavior. Formal P0-P7 acceptance remains **1/8 (12.5%)**; P8-119 remains a candidate.

## P8-120 candidate - canonical plan completion barrier through production startup

The deterministic production-startup fixture exercises recovery through the canonical planner and `ToolRouter`: the first plan's read-only dependency fails, and the following model request sees its failed observation. When the model attempts an early final answer, the completion verifier blocks it, persists fixed `plan_completion_feedback`, and emits no `turn.completed` at that point. The next model request sees both the failure and feedback, proposes a different plan at revision 2 based on revision 1, and the canonical planner executes its alternate read-only `jobs.get` tool. The successful result is persisted before the replacement plan's `completion_proposed` control; both appear in the final model context, after which the completion gate accepts the answer and the turn emits `turn.completed`. The outer bootstrap and canonical runtime share the same fixture pool.

Focused Worker `production-bootstrap` validation passed **14/14**, Worker `tsc --noEmit --skipLibCheck` and `git diff --check` passed. This proves the structural completion gate only: success-criteria text is not semantically evaluated. The fixture uses an in-memory event/item store, fake SQL pool, stub queue/recovery factories, and deterministic model/read tools; it does not prove live PostgreSQL/RLS, Redis/BullMQ, provider, process-restart, or staging behavior. Formal P0-P7 acceptance remains **1/8 (12.5%)**; P8-120 remains a candidate.

## P8-121 candidate - disposable Redis/BullMQ Turn dispatch subgate

A new opt-in integration fixture starts the default Turn consumer and recovery scanner through `startProductionAgentRuntime` and `createProductionWorkerBootstrap`, persists a Turn dispatch intent in a deterministic SQL-shaped fixture, and observes the real BullMQ Worker consume the generation-0 job from Redis. It closes the bootstrap, re-presents the same session/Turn outbox as pending, creates a second production bootstrap/Worker against the same Redis DB, and observes recovery deliver generation 1. The terminal Turn lease fence rejects the second claim as `lease_not_available`, and the deterministic executor remains single-run with the original user/session/owner and lease version.

CI provides this test a one-job `redis:7.4-alpine` service and explicitly enables the Redis gate on loopback DB 15. Local runs skip unless the disposable-service opt-ins and restricted URL are supplied; the test never falls back to the normal Worker Redis setting. After four fixture-specific CI failures were repaired (lineage SQL matching, strict payload shape, scoped open-session lookup, and re-fetching the completed BullMQ Job before reading its result), the full [CI run 35814598937](https://github.com/YuanshuoDu/applymate-jobcopilot/actions/runs/35814598937) passed on `b137bb3b`. The disposable Turn Redis/BullMQ subgate therefore has CI evidence for bootstrap reconstruction, recovery dispatch, and the terminal lease fence.

This subgate uses fake SQL state and reconstructs Worker/bootstrap objects inside one test process. It does not prove PostgreSQL commit/RLS behavior, an OS process crash/restart, cross-host recovery, full accepted-command-to-Worker AC1, provider/tool execution, or production/staging readiness. Formal P0-P7 acceptance remains **1/8 (12.5%)**; P8-121 remains a candidate.

## P8-122 candidate - PostgreSQL subagent lease and attempt fencing

A new opt-in integration fixture seeds two users and several session/turn/task trees in the existing disposable PostgreSQL CI database, then calls the real `PgSubagentTaskStore` through a non-login, non-superuser, non-`BYPASSRLS` role. Because CI applies Prisma migrations but not deployment-gated `apps/web/prisma/rls/enable.sql`, the fixture creates a test-local `app_current_user_id()` helper and the equivalent tenant policies only in the disposable database, then asserts RLS is active on all three protected tables. It checks that RLS and explicit scope predicates reject a foreign owner, wrong session, and wrong root/turn lineage; a valid task can be claimed and released/reclaimed; and stale owner or attempt finishes cannot overwrite the current lease, while the current attempt can finish. The test uses the existing migrations and CI-only `AGENT_RUNTIME_PG_TEST_URL`; without that exact disposable CI gate it skips and cannot fall back to a configured application database. Its per-owner test pool supplies the tenant setting, so this proves the store's SQL under an established tenant context, not production startup's tenant-context binding.

The focused local Vitest command safely reported **1 skipped** because the disposable database is unavailable here. Worker `tsc --noEmit --skipLibCheck` and whitespace checks passed. The first CI attempt found that `app_current_user_id()` is not part of the Prisma migration set; the fixture now installs its own disposable-database helper and policies. The initial passing implementation run was [CI 35841483330](https://github.com/YuanshuoDu/applymate-jobcopilot/actions/runs/35841483330) on `86c65c9e`; the current-head rerun is recorded below. The test-local helper and policies exist only in the disposable database, and the per-owner pool supplies the tenant setting; production startup tenant-context binding remains unproven. This covers a bounded storage-fencing slice only; it does not establish full P1 or stage acceptance. P8-122 remains a candidate and formal P0-P7 acceptance remains **1/8 (12.5%)**.

## P8-123 candidate - disposable Redis/BullMQ child execution and parent wake

A new opt-in integration fixture starts the production bootstrap and subagent recovery scanner against disposable Redis DB 15. It persists a child dispatch before startup and requires the real scanner to publish that outbox entry to the real BullMQ Worker. The Worker runs the production child executor with deterministic model and tool adapters; the child makes one `jobs.search` call, persists its result, and completes. The production durable-wait resolver then matches that child result, marks the parent wait ready, queues the suspended parent Turn, writes its resume event, and persists the Turn dispatch outbox row. The fixture uses deterministic in-memory task and SQL-shaped database doubles for the non-Redis state.

The queue module is dynamically imported only after the integration test installs its Redis mock, so the production Worker connection is the explicitly validated loopback Redis connection. Local Vitest safely skips the real-Redis case unless the integration opt-ins and restricted URL are set. The first disposable-Redis run exposed a fixture contract mismatch: it passed `TurnExecutionStore` (`identity`) where `createProductionChildExecutor` requires `TurnEngineStore` (`owner`), causing item persistence to fail before tool dispatch. The fixture now uses the correct store contract without an unsafe cast and includes an always-on manager-issued-lease test. Focused Worker validation passed **57 tests**, Worker `tsc --noEmit` passed, and the initial passing implementation run was [CI 35841483330](https://github.com/YuanshuoDu/applymate-jobcopilot/actions/runs/35841483330) on `86c65c9e`; the current-head rerun is recorded below.

The child test uses deterministic in-memory task and SQL-shaped state for non-Redis persistence. It does not prove an OS process crash/restart, cross-host recovery, provider behavior, or full P2 acceptance. P8-123 remains a candidate and formal P0-P7 acceptance remains **1/8 (12.5%)**.

## 2026-09-23 current-head acceptance refresh

On current PR #497 head `e52dc3c03c2d1e759d85efe70eabaef2bff29724`, [CI run 35842420907](https://github.com/YuanshuoDu/applymate-jobcopilot/actions/runs/35842420907) passed Tests, Build, TypeScript, the disposable PostgreSQL private-result owner-isolation and subagent lease/attempt tests, and the disposable Redis/BullMQ Turn-dispatch and child-dispatch/wait-resume tests. This reran the P8-122 and P8-123 integration subgates on the current head; the earlier `86c65c9e` run above remains historical implementation evidence. The [Agent Workspace browser matrix 35842421011](https://github.com/YuanshuoDu/applymate-jobcopilot/actions/runs/35842421011) passed **44** and skipped **12**.

P0's latest-head CI and Draft-boundary gate is complete. PR #497 remains Draft with `REVIEW_REQUIRED` / `BLOCKED`; the protected Current-dump rehearsal was skipped. P1 and P2 remain partial: `e52dc3c0` does not include PostgreSQL-backed model/tool execution for two child tasks or an OS process crash/restart and resumed parent loop. The Redis child fixture still uses deterministic model/tool adapters and in-memory non-Redis task/SQL state. Formal P0-P7 acceptance remains **1/8 (12.5%)**.
