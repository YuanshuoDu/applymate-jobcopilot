...
4. Continue P3 with a bounded intent dispatcher. It must consume only an accepted, server-normalized plan, keep dependency references separate from parent task lineage, fill task/lease/idempotency/budget values at the runtime boundary, and route read tools or child creation through the existing ToolRouter/coordination executors. Replanning and plan revision persistence come after this dispatch seam; a process-local plan revision is not durable acceptance.
