...
- Focused Worker coverage: 8 tests across frame derivation, memory validation, deterministic formatting, message ordering, fallback, and byte bound assertions. Candidate boundary: no live PG/RLS, restart, Redis/BullMQ, cross-worker, provider/browser, or full E2E validation has been run.
