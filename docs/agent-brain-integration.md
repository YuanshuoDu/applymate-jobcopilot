...
4. Wire the P3-4 execution adapter into the canonical loop behind a separate planning-execution gate. The loop must hydrate input references from runtime-owned observations, persist bounded command outcomes, and map tool failures, control barriers and child handles into the existing TurnEngine states. Replanning and plan revision persistence come after this seam; a process-local plan revision is not durable acceptance.
