...
4. Supply the P3-6a factory with a server-owned bridge that validates accepted plan output, hydrates input references from runtime-owned observations, invokes the P3-4 adapter and persists bounded command outcomes. Tool failures, control barriers and child handles must map into existing TurnEngine states. Replanning and plan revision persistence come after this seam; a process-local plan revision is not durable acceptance.
