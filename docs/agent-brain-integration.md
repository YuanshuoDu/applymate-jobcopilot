...
The P3-7 focused Worker checks passed **27/27** across the bridge, loop and canonical-state suites; the agent-protocol event check passed **2/2**, the protocol build and Worker TypeScript (including the shared build) passed, and `git diff --check` passed. This is a submitted candidate, not full P3 or phase completion: no PostgreSQL/RLS, provider, queue, migration, restart or real child-to-parent recovery was run. Individual event appends can leave a partial prefix before a later append failure; replay intentionally skips the plan hook to avoid repeating side effects, so atomic batch append or replay-only materialization remains required before crash-recovery acceptance. Durable plan revisions, outbox delivery and recovery remain open.
...
4. Harden the P3-7 bridge with atomic plan-observation batch persistence or replay-only materialization for partial append recovery. Then add durable plan revisions, outbox/queue delivery and recovery while keeping tool failures, control barriers, child handles and approval/user/dependency waits mapped into existing TurnEngine states. Replanning and plan revision persistence remain separate from the current process-local revision and are required for durable acceptance.
