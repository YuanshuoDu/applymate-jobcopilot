...
2. Register the reviewed child model/tool executor in a gated production bootstrap, using the persisted tree-budget store and owner-neutral execution store. Keep the gate disabled until the additive migration is applied and the final integration path is ready; a default startup must not query a missing tree-budget table.
