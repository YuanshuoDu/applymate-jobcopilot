...
- This slice preserves clean joins and existing wait/approval behavior. Server enforcement for mixed or multiple tool-call batches and the fixed model instruction remain P4-72B work. Candidate boundary: no live PostgreSQL/RLS transaction, canonical restart/replay process, Redis/BullMQ delivery, cross-worker race, provider/browser behavior, or complete child-parent E2E was run.
