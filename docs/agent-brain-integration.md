...
- New retry facts, input, and canonical `agent.turn.dispatch` outbox intent use the existing atomic command transaction. Repeated client identities return the original `CommandResult` without creating another Turn, Input, Item, Event, or Outbox; the retry target remains terminal and is never promoted to active.
