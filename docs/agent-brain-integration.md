...
4. Continue P3 with a runtime execution adapter behind the existing ToolRouter and coordination executor. It must consume only the P3-3 commands, preserve dependency references separately from parent task lineage, and let the runtime own task/lease/idempotency/budget admission and observations. Replanning and plan revision persistence come after that adapter; a process-local plan revision is not durable acceptance.
