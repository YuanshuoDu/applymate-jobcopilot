...
4. Extend the P3-6b server-owned bridge with runtime-owned input-reference hydration and bounded outcome persistence. Keep tool failures, control barriers, child handles and approval/user/dependency waits mapped into existing TurnEngine states; then add durable plan revisions, outbox/queue delivery and recovery. Replanning and plan revision persistence come after the current bridge seam; its process-local revision and loop observations are not durable acceptance.
