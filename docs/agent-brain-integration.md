...
CI now provides this test a one-job `redis:7.4-alpine` service and explicitly enables the Redis gate on loopback DB 15. Local runs skip unless the disposable-service opt-ins and restricted URL are supplied; the test never falls back to the normal Worker Redis setting. In this checkout the focused command safely reported **1 skipped** because no disposable Redis was configured; Worker TypeScript passed. The first CI attempt stopped at the deterministic SQL fixture's multiline lineage-query matcher; the second reached the disposable Redis test but timed out before BullMQ enqueue because the dispatch payload included an extra `userId` field rejected by the strict Turn payload schema. The third reached real Redis/BullMQ enqueue and Worker consumption, then failed when the SQL fixture did not answer the production session/user/control-gate lease query. The fixture now models that scoped open-session result. The fourth reached first Worker completion, reconstructed the bootstrap and recovered the pending outbox through the second Worker, and observed the second job complete; only the skipped-lease `returnvalue` assertion failed because the helper held a BullMQ Job snapshot fetched before completion. `getState()` refreshed the Redis state but not that object's cached `returnvalue`; the helper now re-fetches the completed job. A fresh CI run is required before this subgate can pass.
