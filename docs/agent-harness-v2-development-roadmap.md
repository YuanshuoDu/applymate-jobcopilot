...
The local focused invocation skipped its one integration test because no disposable Redis was configured; Worker TypeScript and `git diff --check` passed. The first CI attempt stopped in the fake SQL fixture's multiline lineage-query matcher. The second reached the disposable Redis test but timed out before BullMQ enqueue because the payload included an extra `userId` field rejected by the strict Turn payload schema. The third reached real Redis/BullMQ enqueue and Worker consumption, then failed because the fake SQL fixture did not model the production session/user/control-gate lease query. The fourth reached first Worker completion, reconstructed the bootstrap and recovered the pending outbox through the second Worker, and observed the second job complete; the skipped-lease `returnvalue` assertion alone failed because the helper retained a BullMQ Job snapshot fetched before completion. `getState()` refreshed Redis state but not that object's cached result; the helper now re-fetches the completed job. A fresh CI run is required before this subgate can pass. The fixture does not prove real PostgreSQL/RLS, OS process crash/restart, cross-host recovery, complete control-plane AC1, provider execution, or production/staging readiness. P8-121 remains a candidate; formal stage acceptance remains 1/8 (12.5%).
